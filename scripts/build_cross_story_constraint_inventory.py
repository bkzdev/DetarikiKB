#!/usr/bin/env python3
"""Stage Aのcross-story relative_order候補を非判定inventoryへ集計する。

Exit codes:
    0: 入力がすべてvalid（候補の有無・未解決targetは失敗にしない）
    1: invalidまたはskipped inputあり
...
from agents.extractor.cross_story_constraint_inventory import (  # noqa: E402
...
REPORT_SCHEMA_PATH = (
    _PROJECT_ROOT / "schemas" / "cross_story_constraint_inventory.schema.json"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "EVENTのStage A episode_extractionにあるcross-story "
            "relative_order候補を、判定・変換せず内部inventoryへ集計します"
        )
    )
    parser.add_argument(
        "--input",
...
def _build_report(
    ...
    inventory = build_cross_story_constraint_inventory(valid_documents)
    status = "invalid_input" if invalid_count or skipped_inputs else "passed"
    report = {
        "schemaVersion": "0.1",
...
def main() -> int:
    ...
    try:
        ...
        # reportは内部ID・pathを含む。例外本文経由でもstdout/stderrへ漏らさない。
        print(
            "[エラー] Cross-story constraint inventoryに失敗しました", file=sys.stderr
        )
        return 2

    if not args.quiet:
        print(
            "[Cross-story constraint inventory] "
            f"status={report['status']} "
            f"resolved={report['resolvedInputFiles']} "
            f"valid={report['validInputs']} "
            f"invalid={report['invalidInputs']} "
            f"in_scope={report['inScopeDocumentCount']} "
            f"observations={report['crossStoryCandidateObservationCount']} "
            f"story_pairs={report['distinctStoryPairCount']} "
            f"same_story={report['sameStoryCandidateCount']} "
            f"unresolved={report['unresolvedTargetCount']} "
            f"ambiguous={report['ambiguousTargetStoryCount']} "
            f"invalid_candidates={report['invalidRelativeCandidateCount']}"
        )
        if args.report_output:
            print("[Cross-story constraint inventory] report_written=true")
