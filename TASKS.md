...
2. `Merged_Knowledge_Design.md` §13 の順で Stage B 実装に進む（着手前にユーザーへ確認する）
   1. ruff known issues cleanup（`TASKS.md` §4 Known Issuesに記載済みの既存ruffエラー解消）
...
- `uv run ruff check .` に既存エラーが多数ある（`feature/extractor-semantic-validation`時点で約80件。内訳: 既存73件 + `scripts/extract_story.py`/`scripts/validate_extraction_json.py` の `E402`（sys.path追加後のimportで`normalize_story.py`と同じ既存パターン）+ 両スクリプトdocstring/epilogの既存長行）。新規PRで無関係な既存エラーまで巻き取って直す必要はないが、自分が触ったファイル内で自分が書いた新規コードにエラーを残さないこと（`agents/extractor/validator.py`はクリーン）。real data dry-run trial時点で`scripts/normalize_story.py`・`scripts/check_script_compatibility.py`にもC901（複雑度）/E501（長行）/F841（未使用変数）/E402の既存エラーが確認された（`check_file`/`build_markdown_report`/`main`のcomplexity超過等）。今回のPRで触った行（絵文字除去のみ）はいずれも対象外で新規エラーは無い。
