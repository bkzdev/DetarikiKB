...
- merge engine 複数入力対応: 実装完了。`feature/merge-multiple-inputs`ブランチで作業中
...
1. `feature/merge-multiple-inputs` の PR を作成し、レビュー・マージする
2. マージ後、`Merged_Knowledge_Design.md` §13 の順で Stage B 実装に進む（着手前にユーザーへ確認する）
   ...
   2. `schemas/merged_knowledge_collection.schema.json`作成の要否判断（collection構造が安定してから）
