...
1. real data dry-run trialで見つかった既知の課題への対応（着手前にユーザーへ確認する。詳細は`docs/runbooks/Real_Data_Dry_Run_Result_Template.md` §3-4）
   ...
   3. `check_script_compatibility.py`単体実行と`normalize_story.py --check-compat`経由の判定差異の解消（`feature/compatibility-check-consistency`。根本原因は特定済み: `agents/parser/normalizer.py`の`_build_compatibility_report`が`newSpeechCommands`を常にハードコードで空配列にしており、`check_script_compatibility.py`の`_is_speech_candidate`ヒント判定を呼び出していない。`compat`ステータスも`warning`/`compatible`の2値のみで`needs_update`/`blocked`相当が無い。修正には`agents/parser/`へのconfig/script_commands.yaml読み込み配線が必要）
   4. `--check-compat`のレポート出力先をオプション化するか、既定動作として明示的にドキュメント化する
   5. 選択肢（`branch`/`#if`）を含む実データでの追加dry-run実施
...
- `check_script_compatibility.py`単体実行と`normalize_story.py --check-compat`経由でNormalized JSONに埋め込まれる`compatibilityReport`の判定が食い違う（根本原因: `agents/parser/normalizer.py`の`_build_compatibility_report`が`newSpeechCommands`を常に空配列でハードコードし、`compat`ステータスも`warning`/`compatible`の2値のみ）。`feature/compatibility-check-consistency`で対応予定。
