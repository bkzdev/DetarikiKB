...
- `ItemCandidate` / `LoreCandidate` / `EventCandidate` 最小抽出（`feature/item-lore-event-candidate-extraction`）: 実装完了、PR準備中
  - `agents/extractor/extractor.py`: 明示的なitemId/itemName（dialogue/monologue/narration/choice Block、stage_direction Block）から `_build_item_candidates` でItemCandidateを生成
  - `agents/extractor/extractor.py`: 明示的なloreId/termName（dialogue/monologue/narration/choice Blockのみ、最も保守的）から `_build_lore_candidates` でLoreCandidateを生成。stage_direction・speakerAssignments経由の抽出は行わない
  - `agents/extractor/extractor.py`: 明示的なeventId/eventName（dialogue/monologue/narration/choice Block、stage_direction Block）から `_build_event_candidates` でEventCandidateを生成
  ...
  - `agents/extractor/models.py`: `ItemCandidateAccumulator`/`LoreCandidateAccumulator`/`EventCandidateAccumulator`、confidence/sourceType定数を追加
  - `tests/extractor/test_item_lore_event_candidate_extraction.py`（新規17件）
...
1. `feature/item-lore-event-candidate-extraction` の PR を作成し、レビュー・マージする
