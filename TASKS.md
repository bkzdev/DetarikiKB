...
- Timeline 最小merge: 実装完了。`feature/merge-timeline`ブランチで作業中
...
1. `feature/merge-timeline` の PR を作成し、レビュー・マージする
2. マージ後、`Merged_Knowledge_Design.md` §13 の順で Stage B 実装に進む（着手前にユーザーへ確認する）
   1. manual override loader
   2. merge report強化（conflicts/unresolvedの詳細集計、`data/extracted/reports/merge_report.json`への出力等）
   3. relationshipType taxonomyの本格整理（`docs/architecture/04_Knowledge_Graph/Relationships.md`確定）
   4. EventCandidateのparticipant/location解決（Relationship mergeで作ったcandidate対応表ロジックを再利用）
   5. timeline contradiction detection（順序整合性の本格検証）
   6. `entities`配下の`schemas/merged_knowledge.schema.json`への`$ref`接続（cross-file $ref方針の決定待ち）
