...
1. `feature/relationship-type-taxonomy` の PR を作成し、レビュー・マージする
2. マージ後、`Merged_Knowledge_Design.md` §13 の順で Stage B 実装に進む（着手前にユーザーへ確認する）
   1. canonical ID方針整理（手動運用フローの確立。OD-001ローマ字表記ルール等の未確定事項の解消）
   2. real data dry-run procedure（実データでの試験運用手順の整理）
