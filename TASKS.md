...
- merge engine skeleton: 実装完了（最小スコープ）。`feature/merge-engine-skeleton`ブランチで作業中
  ...
  - スコープを最小化: **単一入力ファイルのみ対応**（複数ファイル・ディレクトリ入力・`--overrides`予約引数は今回見送り、TASKS.mdへ次アクションとして記録）
...
1. `feature/merge-engine-skeleton` の PR を作成し、レビュー・マージする
2. マージ後、`Merged_Knowledge_Design.md` §13 の順で Stage B 実装に進む（着手前にユーザーへ確認する）
   1. merge engineの複数ファイル・ディレクトリ入力対応（今回skeletonでは単一ファイルのみ）
   2. character/location/organization merge の最小実装（merge key: existing*Id等） → item/lore/event merge → relationship merge → timeline aggregation → manual override loader → report generator
   3. `schemas/merged_knowledge_collection.schema.json`作成の要否判断（collection構造が安定してから）
