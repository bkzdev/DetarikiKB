...
1. `feature/manual-override-loader` の PR を作成し、レビュー・マージする
2. マージ後、`Merged_Knowledge_Design.md` §13 の順で Stage B 実装に進む（着手前にユーザーへ確認する）
   1. merge report強化（conflicts/unresolvedの詳細集計、`data/extracted/reports/merge_report.json`への出力等）
   2. relationshipType taxonomyの本格整理（`docs/architecture/04_Knowledge_Graph/Relationships.md`確定）
   3. timeline contradiction detection（順序整合性の本格検証）
   4. canonical ID方針整理（手動運用フローの確立。OD-001ローマ字表記ルール等の未確定事項の解消）
   5. Wiki出力設計
