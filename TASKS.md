...
- **character profile import batch 001**: 人間が確認した実キャラクターの公式プロフィールデータを`knowledge/dictionaries/character_profiles.yaml`へ投入する（`scripts/validate_character_profiles.py`でconfirmed済みcharacterIdとの整合性を検証してから反映）。まず`scripts/import_character_profiles_from_wiki.py`で正しいメンバー一覧テーブルURLに対するdry-runを成功させる必要がある
- **character profile renderer section**: `agents/wiki_generator/renderer.py`のCharacter pageに、`character_profiles.yaml`を参照する「基本プロフィール」sectionの実装を追加する（`Wiki_Output_Design.md` §9.4の設計に従う）
