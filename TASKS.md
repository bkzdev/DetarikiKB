...
- merged knowledge collection schema作成: 実装完了。`feature/merged-knowledge-collection-schema`ブランチで作業中
...
1. `feature/merged-knowledge-collection-schema` の PR を作成し、レビュー・マージする
2. マージ後、`Merged_Knowledge_Design.md` §13 の順で Stage B 実装に進む（着手前にユーザーへ確認する）
   1. character/location/organization merge の最小実装（merge key: existing*Id等） → item/lore/event merge → relationship merge → timeline aggregation → manual override loader → report generator
   2. `entities`配下の`schemas/merged_knowledge.schema.json`への`$ref`接続（本格candidate merge実装後、上記TODO参照）
