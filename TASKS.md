...
1. **evidence-index-renderer-integration**: Evidence page生成・Story Summary/Episode Summary evidenceRefsのリンク化・Story pageからの導線を実装する
2. **evidence-index-generation-dry-run**: Normalized Story JSON/Extraction ResultからEvidence index候補を生成し、workspace配下でdry-runする
...
5. **internal-review-evidence-packet-design**: raw textを含みうるInternal Review Evidence Packetの詳細設計を検討する
...
- **evidence-index-renderer-integration** / **evidence-index-generation-dry-run**（Next参照）
