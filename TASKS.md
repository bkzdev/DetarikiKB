...
1. **evidence-index-generation-dry-run**: Normalized Story JSON/Extraction ResultからEvidence index候補を生成し、workspace配下でdry-runする
...
5. **episode-page-evidence-linking-review**: Episode pageへのEvidence/Summary導線追加可否をレビューする（本PRではEpisode pageは無変更のまま）
...
- **evidence-index-generation-dry-run**（Next参照）
- **internal-review-evidence-packet-design**（Next参照）
- **episode-page-evidence-linking-review**（Next参照）
