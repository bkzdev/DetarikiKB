...
1. `feature/canonical-id-policy` の PR を作成し、レビュー・マージする
2. マージ後、`Merged_Knowledge_Design.md` §13 の順で Stage B 実装に進む（着手前にユーザーへ確認する）
   1. real data dry-run procedure（実データでの試験運用手順の整理）
   2. no-invisible-unicode-check（PRマージ前のUnicode隠し文字監査手順のドキュメント化。これまでスクラッチスクリプトで都度実施してきたものを定型化する）
