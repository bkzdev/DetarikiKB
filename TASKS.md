...
- Stage B schema作成: 初版完了。`feature/merged-knowledge-schema`ブランチで作業中（Python merge engineは未着手）
...
1. `feature/merged-knowledge-schema` の PR を作成し、レビュー・マージする
2. マージ後、`Merged_Knowledge_Design.md` §13 の順で Stage B 実装に進む（着手前にユーザーへ確認する）
   1. merge engine skeleton（`agents/merger/` 仮、Stage Aファイル読み込み・検証ゲート・空merged構造出力・merge_report骨格）
   2. character/location/organization merge → item/lore/event merge → relationship merge → timeline aggregation → manual override loader → report generator
