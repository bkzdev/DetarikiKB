...
- Character / Location / Organization 最小merge: 実装完了。`feature/merge-character-location-organization`ブランチで作業中
...
1. `feature/merge-character-location-organization` の PR を作成し、レビュー・マージする
2. マージ後、`Merged_Knowledge_Design.md` §13 の順で Stage B 実装に進む（着手前にユーザーへ確認する）
   1. Item/Lore/Event merge の最小実装（Character/Location/Organizationと同じ`entity_base.py`パターンを踏襲）
   2. Relationship merge（両端解決済みのみ昇格するゲート条件、Merged_Knowledge_Design.md §6）
   3. Timeline aggregation（kind別集約、§7）
   4. manual override loader
   5. `entities`配下の`schemas/merged_knowledge.schema.json`への`$ref`接続（cross-file $ref方針の決定待ち）
