...
1. `feature/real-data-dry-run-procedure` の PR を作成し、レビュー・マージする
2. マージ後、`Merged_Knowledge_Design.md` §13 の順で Stage B 実装に進む（着手前にユーザーへ確認する）
   1. no-invisible-unicode-check（PRマージ前のUnicode隠し文字監査手順のドキュメント化。これまでスクラッチスクリプトで都度実施してきたものを定型化する）
   2. timeline contradiction detection（順序整合性の本格検証）
   3. Wiki出力設計
   4. real data dry-run trial（`docs/runbooks/Real_Data_Dry_Run.md`に従った実データでの実際の試験運用。手順自体はこのPRで整備済み）
