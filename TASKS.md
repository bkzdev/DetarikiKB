...
- `CharacterCandidate` 最小抽出（`feature/character-candidate-extraction`）: 実装完了、PR準備中
  ...
  - `agents/extractor/models.py`: `CharacterCandidateAccumulator`、confidence/sourceType定数を追加
  - `tests/extractor/test_character_candidate_extraction.py`（新規12件）、`tests/extractor/test_extractor_skeleton.py`の空配列チェックを更新（charactersは対象外に）
...
1. `feature/character-candidate-extraction` の PR を作成し、レビュー・マージする
2. マージ後、以下のいずれかに着手する（優先順位は未確定、着手前にユーザーへ確認する）
   - choice内話者も含めたCharacterCandidate抽出への拡張
