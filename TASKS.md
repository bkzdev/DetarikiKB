...
- **Character profile dictionary design**: `knowledge/dictionaries/characters.yaml`はID解決用辞書（`sourceCharacterId`/`characterId`/`displayName`/`aliases`/`status`/`notes`）に用途を限定し続け、公式プロフィール情報（読み仮名・所属・身長・誕生日・血液型・CV・自己紹介文・特記事項等）は別ファイル `knowledge/dictionaries/character_profiles.yaml`（候補パス）として設計する。characters.yamlにプロフィール項目を詰め込まない。プロフィールは**confirmed済みcharacterId**にのみ紐づけ、AI抽出情報（Stage A/B由来）とは混ぜず「公式プロフィール情報」として扱う。読み仮名（kana/romaji）は名前解決用のaliases/searchAliasesとは別軸のプロフィール寄り項目として扱い、両者の関係は本タスクでは設計しない（将来検討）
- **character_profiles.yaml schema design**: 候補項目 `characterId`（confirmed済みcharacterIdへの参照）/`displayName`/`nameReading`（`kana`/`romaji`）/`affiliation`（所属）/`height`（身長）/`birthday`（誕生日）/`bloodType`（血液型）/`cv`（声優）/`selfIntroduction`（自己紹介文）/`profileNotes`（`likes`/`dream`/`fear`/`other`等の特記事項）。JSON Schema化・validatorの要否・`characters.yaml`とのキー突き合わせ方式（characterId外部キー想定）を設計する
- **Wiki character profile section design**: Wiki Character page（`docs/architecture/07_Wiki/Wiki_Output_Design.md` §9.4）に、`character_profiles.yaml`由来の情報を表示する「基本プロフィール」sectionを追加する設計を行う。既存の`## Summary`/`## Aliases`/`## Evidence`等のAI抽出・merge由来セクションとは明確に区別し、公式プロフィール情報であることが分かる見出し・出典表記にする
  - **Non-goals（これら3タスク自体は設計検討のみ、今回のTASKS.md追記でも実装は行わない）**: `character_profiles.yaml`の実装・スキーマファイル作成、公式プロフィールデータの投入、`characters.yaml`へのプロフィール項目追加、キャラクター辞書のconfirmed化、AI推測によるプロフィール作成、実データ由来プロフィール情報のcommit、Wiki rendererへのプロフィール表示実装
