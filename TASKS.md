...
- **character profile import batch 001**: 人間が確認した実キャラクターの公式プロフィールデータを`knowledge/dictionaries/character_profiles.yaml`へ投入する（`scripts/validate_character_profiles.py`でconfirmed済みcharacterIdとの整合性を検証してから反映）
