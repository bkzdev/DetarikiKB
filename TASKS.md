...
1. `feature/no-invisible-unicode-check` の PR を作成し、レビュー・マージする
2. マージ後、`Merged_Knowledge_Design.md` §13 の順で Stage B 実装に進む（着手前にユーザーへ確認する）
   1. real data dry-run trial（`docs/runbooks/Real_Data_Dry_Run.md`に従った実データでの実際の試験運用。手順自体は前PRで整備済み）
   2. ruff known issues cleanup（`TASKS.md` §4 Known Issuesに記載済みの既存ruffエラー解消。今回のPRでも対象外にした既存15件を含む）
   3. timeline contradiction detection（順序整合性の本格検証）
   4. Wiki出力設計
   5. relationshipType taxonomyの本確定（`docs/architecture/04_Knowledge_Graph/Relationships.md`。今回はまだ暫定taxonomy・正規化レイヤーのみ）
   6. canonical ID辞書（`knowledge/dictionaries/*.yaml`、`Merged_Knowledge_Design.md` §2.4）の実装（今回はpolicy/helper/validationのみ）
   7. EventCandidateのparticipant/location解決（Relationship mergeで作ったcandidate対応表ロジックを再利用）
   8. `entities`配下の`schemas/merged_knowledge.schema.json`への`$ref`接続（cross-file $ref方針の決定待ち）
   9. merge report自体の生成物出力（`data/extracted/reports/merge_report.json`への書き出し。§11.2）
