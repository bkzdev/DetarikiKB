...
- **character profile import batch 001**: 人間が確認した実キャラクターの公式プロフィールデータを`knowledge/dictionaries/character_profiles.yaml`へ投入する（`scripts/validate_character_profiles.py`でconfirmed済みcharacterIdとの整合性を検証してから反映）。実WIKI dry-runでのURL特定・candidate生成確認は完了済み。人間がcandidate fileの内容（matched 6件）を確認してから進める
