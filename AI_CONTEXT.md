...
- `docs/architecture/06_AI/Internal_Review_Evidence_Packet_Design.md`（内部ID⇔公開ID mappingとraw本文をPublic Evidence Indexから分離してローカルreviewするPacketの詳細設計。固定rootは`workspace/review_packets/evidence/`、commit禁止、Public側とは別schema/validator/CLI、既定14日のephemeral retention、human review noteとpromotion権限を分離する。Phase 5.2としてmanifest/story/selection/safe validation reportの4 schemaと、既存bundleを変更しないvalidatorは実装済み。外部source candidate/Normalized Storyとのcross-checkを行うgenerator、およびcleanup運用CLIは未実装）
