...
Extraction Phase関連（設計完了。`schemas/extraction.schema.json`・`agents/extractor/`の最小skeleton・semantic validation・`CharacterCandidate`/`LocationCandidate`/`OrganizationCandidate`/`ItemCandidate`/`LoreCandidate`/`EventCandidate`/`RelationshipCandidate`最小抽出・Extractor内部のファイル分割はmainへマージ済み、`TimelineCandidate`最小抽出は`feature/timeline-candidate-extraction`でPR準備中）。
...
次の自然な一歩は `TASKS.md` の Next Actions（`feature/timeline-candidate-extraction`のPR以降）に従う。着手前に以下を決めておく。
