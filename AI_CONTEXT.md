...
- `docs/architecture/06_AI/Internal_Review_Evidence_Packet_Design.md`（内部ID⇔公開ID mappingとraw本文をPublic Evidence Indexから分離してローカルreviewするPacketの詳細設計。固定rootは`workspace/review_packets/evidence/`、commit禁止、Public側とは別schema/validator/CLI、既定14日のephemeral retention、human review noteとpromotion権限を分離する。Phase 5.2としてmanifest/story/selection/safe validation reportの4 schemaと、既存bundleを変更しないvalidatorは実装済み。Phase 5.3としてNormalized Story/Public-safe candidate/mappingをcross-checkしてsafe report付きbundleをtemporary directory→validator→no-clobber atomic publishで生成するgeneratorは実装済み。projection reportは現行Markdownにschema/digest欄がないため機械入力にしない。cleanup/inventory/runbook、実データPacket生成、context opt-in、review note/promotionは未実装）
