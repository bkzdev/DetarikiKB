...
- `docs/architecture/07_Wiki/Public_Publishing_Workflow_Decision.md`（公開基盤・generator・trigger・deploy gate・rollbackのdocs-only判断枠、Status: Proposed。GitHub Pages custom Actions、現行MkDocs / Materialの暫定lock＋公開前Zensical合成dual-build、manual production、全安全gate、既知正常SHA再配備を推奨する。採択・workflow実装・外部接続・実データ公開・deployは未実施）
