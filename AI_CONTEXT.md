...
`agents/extractor/` は最小skeleton（Normalized Story JSONから`episode_extraction`の構造とevidenceIndexを生成、候補配列は空、LLM呼び出しなし）まで実装済み・mainへマージ済み。さらにsemantic validation（`agents/extractor/validator.py`: evidenceIds実在確認、duplicate candidate id検出、empty evidenceIndex検出、extractionRun整合性確認、relationship基本チェック。`scripts/validate_extraction_json.py --semantic`から利用可能）を`feature/extractor-semantic-validation`ブランチでPR準備中。LLM呼び出し本体・provider連携・prompt設計・`CharacterCandidate`本格抽出は、CLAUDE.mdの方針により明示的な指示があるまで着手しない。
...
Extraction Phase関連（設計完了。`schemas/extraction.schema.json`・`agents/extractor/`の最小skeletonはmainへマージ済み、semantic validationは`feature/extractor-semantic-validation`でPR準備中）。
...
Parser本体（`agents/parser/`）、`schemas/extraction.schema.json` 系、`agents/extractor/` の最小skeleton・semantic validationへの再着手は不要（完了済み、§3.1）。

次の自然な一歩は `TASKS.md` の Next Actions（`feature/extractor-semantic-validation`のPR以降）に従う。着手前に以下を決めておく。
