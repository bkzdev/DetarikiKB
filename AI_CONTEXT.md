...
次の自然な一歩は `TASKS.md` の Next Actions（real data dry-run trialで見つかった既知の課題対応（演出コマンド辞書拡充・キャラクター辞書拡充・互換性チェック判定差異の解消等）→ `Merged_Knowledge_Design.md` §13のPR分割案に従い ruff known issues cleanup → timeline contradiction detection → Wiki出力設計 → relationshipType taxonomy本確定 → canonical ID辞書実装）に従う。着手前に以下を守る。
