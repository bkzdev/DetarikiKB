...
Extraction Phase関連（設計完了。`schemas/extraction.schema.json`・`agents/extractor/`の最小skeleton・semantic validationはmainへマージ済み、`CharacterCandidate`最小抽出は`feature/character-candidate-extraction`でPR準備中）。
...
Parser本体（`agents/parser/`）、`schemas/extraction.schema.json` 系、`agents/extractor/` の最小skeleton・semantic validation・`CharacterCandidate`最小抽出への再着手は不要（完了済み、§3.1）。

次の自然な一歩は `TASKS.md` の Next Actions（`feature/character-candidate-extraction`のPR以降）に従う。着手前に以下を決めておく。
