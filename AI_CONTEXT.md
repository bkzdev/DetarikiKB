...
次の自然な一歩は `TASKS.md` の Next Actions（`feature/no-invisible-unicode-check`のPR以降、`Merged_Knowledge_Design.md` §13のPR分割案に従い、real data dry-run trial → ruff known issues cleanup → timeline contradiction detection → Wiki出力設計 → relationshipType taxonomy本確定 → canonical ID辞書実装）に従う。着手前に以下を守る。
