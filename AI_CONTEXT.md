...
**Stage A candidate extractionは完了。Stage B（Merged Knowledge）は設計書・schema（entity単位）・merge engine（単一/複数入力）完了→現在collection wrapperのschema作成中**。`Merged_Knowledge_Design.md`（設計書）・`schemas/merged_knowledge.schema.json`（8種のmerged entityをoneOf判別）・`schemas/manual_overrides.schema.json`（手動補正ファイル）・`agents/merger/`（`MergeEngine`: 複数ファイル/ディレクトリ/globパターン入力対応の検証ゲート+空collection組み立て、`input_resolver.py`）はmainへマージ済み。merge engineが返すcollection wrapper自体のスキーマ`schemas/merged_knowledge_collection.schema.json`を`feature/merged-knowledge-collection-schema`で作成中（entities配下は現状常に空のため要素は`object`として緩く許容、`merged_knowledge.schema.json`への`$ref`接続は本格candidate merge実装後に着手予定）。本格的なcandidate merge・canonical ID割り当て・manual override適用・relationship merge・timeline aggregationはまだ未着手。`schemas/canonical_knowledge.schema.json`はStage C用の予約placeholder。重要ルール: **Stage A candidateのevidence（sourceType/confidence/evidenceIds/candidate ID/extractionRun）はマージ後も失わない**（schemaでevidenceRefs・sourceCandidatesを最低1件必須にして担保）。LLM呼び出し本体・provider連携・prompt設計は、CLAUDE.mdの方針により明示的な指示があるまで着手しない。
...
Extraction Phase関連（Stage A設計・実装完了。`schemas/extraction.schema.json`・`agents/extractor/`の全8種Candidate最小抽出・semantic validation・Stage A統合レビュー・Stage B設計書`Merged_Knowledge_Design.md`・Stage B entity schema `schemas/merged_knowledge.schema.json`/`schemas/manual_overrides.schema.json`・merge engine（複数入力対応、`agents/merger/`）はmainへマージ済み。merge engine collection wrapper schema `schemas/merged_knowledge_collection.schema.json`は`feature/merged-knowledge-collection-schema`でPR準備中）。
...
次の自然な一歩は `TASKS.md` の Next Actions（`feature/merged-knowledge-collection-schema`のPR以降、`Merged_Knowledge_Design.md` §13のPR分割案に従い、character/location/organization merge最小実装）に従う。着手前に以下を守る。
