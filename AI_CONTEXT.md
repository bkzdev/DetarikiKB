...
**Stage A candidate extractionは完了。Stage B（Merged Knowledge）は設計書・schema（entity単位・collection単位）・merge engine（単一/複数入力）・全8種Candidateの最小merge完了→現在manual override loader実装中**。`Merged_Knowledge_Design.md`（設計書）・`schemas/merged_knowledge.schema.json`（8種のmerged entityをoneOf判別）・`schemas/manual_overrides.schema.json`（手動補正ファイル）・`schemas/merged_knowledge_collection.schema.json`（merge engineのcollection wrapper用）・`agents/merger/`（`MergeEngine`・`entity_base.py`共通処理・`character.py`/`location.py`/`organization.py`/`item.py`/`lore.py`/`event.py`/`relationship.py`/`timeline.py`）はmainへマージ済み。`agents/merger/overrides.py`により、merge後のcollectionへmanual override（displayName/status/canonicalIdの上書き、aliasesの追加・削除、fieldValues経由のnotes追加のみ対応）を適用する実装を`feature/manual-override-loader`で作成中。対象entityの特定は保守的（merged entity id → canonicalId → Stage A candidate idの順、名前一致は絶対に使わない）。適用してもevidenceRefs/sourceCandidates/extractionRunRefs/conflictsは一切変更しない。`schemas/manual_overrides.schema.json`・`schemas/merged_knowledge.schema.json`（個別entity）は無変更（既存フィールドのみで表現）、`schemas/merged_knowledge_collection.schema.json`のMergeReportに任意プロパティ`manualOverrides`を1箇所追加（`--overrides`未指定時は出現せず既存挙動を維持）。canonical ID本格割り当て・高度なconflict解決・timeline contradiction detection・relationshipType taxonomy確定はまだ未着手。`entities`配下の`merged_knowledge.schema.json`への`$ref`接続は引き続き見送り中（PR #18のTODO）。`schemas/canonical_knowledge.schema.json`はStage C用の予約placeholder。重要ルール: **Stage A candidateのevidence（sourceType/confidence/evidenceIds/candidate ID/extractionRun）はマージ後も失わない**（schemaでevidenceRefs・sourceCandidatesを最低1件必須にして担保。manual override適用後も保持されることをテストで確認済み）。LLM呼び出し本体・provider連携・prompt設計は、CLAUDE.mdの方針により明示的な指示があるまで着手しない。
...
次の自然な一歩は `TASKS.md` の Next Actions（`feature/manual-override-loader`のPR以降、`Merged_Knowledge_Design.md` §13のPR分割案に従い、merge report強化 → relationshipType taxonomy整理 → timeline contradiction detection → canonical ID方針整理 → Wiki出力設計）に従う。着手前に以下を守る。
