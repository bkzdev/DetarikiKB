...
Extraction Phase関連（設計完了。`schemas/extraction.schema.json`・`agents/extractor/`の最小skeleton・semantic validation・`CharacterCandidate`/`LocationCandidate`/`OrganizationCandidate`/`ItemCandidate`/`LoreCandidate`/`EventCandidate`最小抽出・Extractor内部のファイル分割はmainへマージ済み、`RelationshipCandidate`最小抽出は`feature/relationship-candidate-extraction`でPR準備中）。
...
Parser本体（`agents/parser/`）、`schemas/extraction.schema.json` 系、`agents/extractor/` の最小skeleton・semantic validation・`CharacterCandidate`/`LocationCandidate`/`OrganizationCandidate`/`ItemCandidate`/`LoreCandidate`/`EventCandidate`/`RelationshipCandidate`最小抽出、Extractor内部のファイル分割への再着手は不要（完了済み、§3.1）。

次の自然な一歩は `TASKS.md` の Next Actions（`feature/relationship-candidate-extraction`のPR以降）に従う。着手前に以下を決めておく。
