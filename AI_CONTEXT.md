...
- `docs/architecture/03_Data_Model/Canonical_Timeline_Promotion_Plan.md`（v0.2 review packetのhuman-confirmedなknown relationを、元edge・全provenance・decision保持の`proposed_canonical_edge`として表すlocal internal / plan-only v0.1契約。検証済みpacketから全適格edgeを決定的にdeep copyするin-memory projectorと、source packet / story pair / expiry / edge 1:1対応を検査するcross-document semantic validatorまで実装済み。CLI / file I/O、canonical artifact preflight / write、promotion executor、実plan、public表示は未実装）
