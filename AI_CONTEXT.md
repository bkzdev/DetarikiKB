...
- `docs/architecture/06_AI/Story_Summary_Design.md`（Story/Episode Summaryのデータ構造・保存場所（`knowledge/summaries/stories/{storyId}.yaml`）・status/review workflow設計。schema/loader/validator実装済み、`scripts/render_wiki.py --story-summaries`によるStory page renderer統合も実装済み。AI要約生成・Episode pageへの表示はまだ未着手）
