...
次の自然な一歩は `TASKS.md` の Next Actions（real data dry-run trialで見つかった既知の課題対応のうち残り（キャラクター辞書拡充・`feature/compatibility-check-consistency`での互換性チェック判定差異の解消・選択肢を含む実データでの追加dry-run。演出コマンド辞書拡充は`script command coverage improvement`で対応済み）→ `Merged_Knowledge_Design.md` §13のPR分割案に従い ruff known issues cleanup → timeline contradiction detection → Wiki出力設計 → relationshipType taxonomy本確定 → canonical ID辞書実装）に従う。着手前に以下を守る。
