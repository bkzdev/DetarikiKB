...
- `docs/architecture/06_AI/Story_Summary_Design.md`（Story/Episode Summaryのデータ構造・保存場所（`knowledge/summaries/stories/{storyId}.yaml`）・status/review workflow設計。schema/loader/validator・Story page renderer統合（`--story-summaries`）・evidenceRefsのEvidence indexへのリンク化・AI要約生成パイプライン・レビュー済みSummaryの公開投入まで実装済み。Episode pageへの表示は未実装だが、`episode-page-evidence-linking-review`で対象Episodeの表示可能なEpisode Summary本文と直下の`evidenceRefs`だけを追加する限定契約を確定し、次PRを`episode-page-summary-evidence-linking`とした。AI要約生成パイプラインの設計は`docs/architecture/06_AI/Story_Summary_Generation_Plan.md`を参照）
...
- `docs/architecture/07_Wiki/Story_Page_Design.md`（Story page中心構造への設計方針。`render_story_page`/`story_page_path`で実装済み、Episode pageは維持。Story pageは表示可能なStory/Episode SummaryとevidenceRefsを表示済み。Episode pageへの限定表示は`episode-page-evidence-linking-review`で方針確定、実装は次PR）
