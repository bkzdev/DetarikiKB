...
PUBLIC_PUBLISHING_DECISION_PATH = (
    PROJECT_ROOT
    / "docs"
    / "architecture"
    / "07_Wiki"
...
def test_public_projection_decision_keeps_implementation_and_publish_gates():
    ...
    assert "公開profileの採択、protected Pages gate" in milestones
...
def test_public_preflight_contract_is_implemented_and_keeps_candidate_state():
    ...
    assert "5入力digest preflight" in tasks


def test_public_renderer_contract_is_implemented_with_reviewed_public_input():
...
def test_public_preview_records_real_data_visual_and_safety_review():
...
def test_public_publishing_decision_is_accepted_and_non_deploying():
    ...
    for required in (
        ...
        "public-safe構造化入力だけを昇格",
        "internal input digestを持つlocal preflight reportは非commit",
        "public PR previewを生成しない",
        "logへlabelや本文を列挙しない",
        "既知SHAで手動再実行",
        "rollback rehearsal",
        "推奨P1〜P8を変更なしで一括採択",
...
def test_first_delegated_timeline_batch_is_recorded_anonymously():
    ...
    assert "合計40関係を反映済み" in milestones
    assert "15回の小規模batch運用を実証済み" in milestones
...
def test_twelfth_delegated_timeline_batch_is_recorded_anonymously():
    tasks = _read(TASKS_PATH)
    promotion = _read(CANONICAL_TIMELINE_PROMOTION_RUNBOOK_PATH)
    milestones = _read(
        PROJECT_ROOT / "docs/architecture/01_Project/Project_Milestones.md"
    ...
    for required in (
        "`codex/canonical-timeline-batch-014`",
        "68 nodes / 36 edges",
        "36 distinct story pair",
        "作業開始時の既存62 nodes / 33 edgesは内容不変",
        "候補1組と、共通チーム名だけで出来事の直接接続がない候補1組",
        "schema error 0",
        "semantic finding 0",
    ):
        assert required in tasks
    ...
    for required in (
        ...
        "同一キャンペーン期間だが厳密な前後を確定できず",
        "既存33 story pairとの重複がない",
        "68 nodes / 36 edges",
        "36 distinct story pair",
        "既存62 nodes / 33 edgesは内容不変",
        "保留候補の自動確定",
        "public projection",
    ):
        assert required in promotion
...
def test_thirteenth_delegated_timeline_batch_is_recorded_anonymously():
    tasks = _read(TASKS_PATH)
    promotion = _read(CANONICAL_TIMELINE_PROMOTION_RUNBOOK_PATH)
    milestones = _read(
        PROJECT_ROOT / "docs/architecture/01_Project/Project_Milestones.md"
    ...
    for required in (
        "`codex/canonical-timeline-batch-015`",
        "69 nodes / 37 edges",
        "37 distinct story pair",
        "作業開始時の既存68 nodes / 36 edgesは内容不変",
        "回次・類似制度だけで具体的な先行事件を一意に結べない候補2組",
        "共通する撮影行為だけで相互参照がない候補1組",
        "schema error 0",
        "semantic finding 0",
    ):
        assert required in tasks
    ...
    for required in (
        "13回目の小規模batch（2026-08-31）",
        "初回の投票企画",
        "既存36 story pairとの重複がない",
        "69 nodes / 37 edges",
        "37 distinct story pair",
        "既存68 nodes / 36 edgesは内容不変",
        "保留候補の自動確定",
        "public projection",
    ):
        assert required in promotion
...
def test_fourteenth_delegated_timeline_batch_is_recorded_anonymously():
    tasks = _read(TASKS_PATH)
    promotion = _read(CANONICAL_TIMELINE_PROMOTION_RUNBOOK_PATH)
    milestones = _read(
        PROJECT_ROOT / "docs/architecture/01_Project/Project_Milestones.md"
    ...
    for required in (
        "`codex/canonical-timeline-batch-016`",
        "70 nodes / 38 edges",
        "38 distinct story pair",
        "作業開始時の既存69 nodes / 37 edgesは内容不変",
        "同じ題材名や参加活動だけで直接の相互参照がない別候補3組",
        "schema error 0",
        "semantic finding 0",
    ):
        assert required in tasks
    ...
    for required in (
        "14回目の小規模batch（2026-08-31）",
        "固有名付き第三アイドルユニット",
        "既存37 story pairとの重複がない",
        "70 nodes / 38 edges",
        "38 distinct story pair",
        "既存69 nodes / 37 edgesは内容不変",
        "保留候補の自動確定",
        "public projection",
    ):
        assert required in promotion
...
def test_fifteenth_delegated_timeline_batch_is_recorded_anonymously():
    tasks = _read(TASKS_PATH)
    promotion = _read(CANONICAL_TIMELINE_PROMOTION_RUNBOOK_PATH)
    milestones = _read(
        PROJECT_ROOT / "docs/architecture/01_Project/Project_Milestones.md"
    ...
    for required in (
        "`codex/canonical-timeline-batch-017`",
        "72 nodes / 40 edges",
        "40 distinct story pair",
        "作業開始時の既存70 nodes / 38 edgesと中間71 nodes / 39 edgesは内容不変",
        "具体的な出来事を接続できない別候補5組",
        "schema error 0",
        "semantic finding 0",
    ):
    ...
    for required in (
        ...
        "固有名付き第二アイドルユニット",
        "既存38 story pairとの重複がない",
        "72 nodes / 40 edges",
        "40 distinct story pair",
        "既存70 nodes / 38 edgesと中間71 nodes / 39 edgesは内容不変",
        "保留候補の自動確定",
        "public projection",
    ):
        assert required in promotion
