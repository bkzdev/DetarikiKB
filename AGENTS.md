...
- 親エージェントは`gpt-5.6-sol`・推論強度`medium`を既定とし、設計、進行管理、統合、最終判断を担当する。複雑な設計判断や難しい原因調査など、品質向上が見込める難所だけ一時的に`high`へ上げる。
...
- 独立コードレビュー・セキュリティレビュー・複雑な設計検証を担当する監査サブエージェントは、節目に限って原則`gpt-5.6-terra`・推論強度`high`で起動する。`xhigh`は常用せず、高リスクな最終差分の監査で測定可能な品質向上が見込める場合、またはユーザーの明示指定がある場合に1回だけ使う。
- 通常は同時に動かすサブエージェントを1体までとし、履歴継承を最小限にして、対象ファイル・確認項目・返却形式を限定する。進捗監査は常時ループさせず、実装と対象テストの完了後、PR提出前、merge後の次工程確認などの節目だけで行う。
- 分割による調整コストが上回る短い作業や、同じファイルを密に編集する作業は無理に委譲しない。
- 具体的な既定値は`.codex/config.toml`を正とし、タスクまたはユーザーがモデル・推論強度を明示した場合はその指定を優先する。

## Codexの検証・使用制限運用

- 編集中は変更領域に対応する対象テストを論理的な編集バッチごとにまとめて実行する。独立監査で差分を確定した後、`docs/runbooks/AI_PR_Playbook.md`の標準検証をコミット前に1回実行する。
- push後はGitHub Actions CIを正として確認し、CIが同じ検証を実行している場合はローカルのフル検証を繰り返さない。merge後はmain同期とstatus確認だけを行う。
