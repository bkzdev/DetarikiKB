...
| M6 公開準備 | 進行中 | 公開範囲、ホスティング、更新・rollback、漏えい検査を決定し、限定公開できる | 公開platform / workflowとZensical移行方針を採択済み。次はgenerator統合と安全な公開入力契約 |
...
3. ~~明示接続候補の初回走査完了を受け、公開用Timelineの目的・表示粒度・unknown/conflict表現を判断し、public projection schema・projector・preflight・rendererを合成fixtureで固定する。~~ 公開profileの採択は2026-09-01に完了し、local visual review、公開platform / workflow、Zensical合成dual-buildと移行判断まで2026-09-02に完了。次はZensical exact pinの段階統合とpublic-safe入力契約を小さいPRで固定する。
