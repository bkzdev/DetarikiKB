...
| M5 Wiki / 閲覧体験 | 進行中 | public-safeなStory/Episode/Character/Evidenceページを一貫生成できる | 基本rendererとLocation / Item page・各indexは実装済み。残entityページ・関係表示・全体目視確認が残る |
...
現在の主対象はM5へ移っている。M4ではCanonical Timelineのschema、semantic check、review packet、promotion plan、安全なlocal executorを完成させ、実データへ合計40関係を反映済みである。M5ではPhase 1のStory / Episode / Character / Evidenceに続き、Phase 2のLocation / Item page・各indexとTop page導線を実装した。

次の切りのよい到達点は次の3つである。

1. Location / Item pageと各indexを合成fixture、strict build、独立監査で確定し、M5 Phase 2の共通実装パターンにする。
2. Organization / Item / Lore / Event pageを依存関係と表示仕様に沿って小さなPRへ分け、Relationship表示はtaxonomy判断後に進める。
