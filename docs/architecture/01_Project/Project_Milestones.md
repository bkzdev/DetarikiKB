...
| M6 公開準備 | 進行中 | 公開範囲、ホスティング、更新・rollback、漏えい検査を決定し、限定公開できる | Zensical dual-buildとCanonical Timeline public input契約を実装済み。前段階の「次はsite manifest / HTML漏えい検査」「次はbuild-only workflow」とmanual production environment gateも完了し、次は匿名合成siteのdeploy / rollback rehearsal |
...
3. ~~明示接続候補の初回走査完了を受け、公開用Timelineの目的・表示粒度・unknown/conflict表現を判断し、public projection schema・projector・preflight・rendererを合成fixtureで固定する。~~ 公開profileの採択は2026-09-01に完了し、Zensical dual-build、public-safe入力、site manifest / rendered exposure gate、合成build-only workflow、manual production workflow / environment gateまで完了。次は匿名合成siteのdeploy / rollback rehearsalを独立PRで固定する。
