...
| M6 公開準備 | 進行中 | 公開範囲、ホスティング、更新・rollback、漏えい検査を決定し、限定公開できる | 匿名合成siteのA→B→A deploy / rollback rehearsalまで完了。次は実データpublic projectionをignored workspaceで生成し、push前に人間レビュー |
...
3. ~~明示接続候補の初回走査完了を受け、公開用Timelineの目的・表示粒度・unknown/conflict表現を判断し、public projection schema・projector・preflight・rendererを合成fixtureで固定する。~~ 公開profileの採択からprotected Pages deploy、匿名合成siteのA→B→A rollback rehearsalまで完了した。次は実データpublic projectionをignored workspaceで生成し、push前人間レビューを行う。
