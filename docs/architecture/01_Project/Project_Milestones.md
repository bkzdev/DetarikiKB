...
| M6 公開準備 | 進行中 | 公開範囲、ホスティング、更新・rollback、漏えい検査を決定し、限定公開できる | Zensical dual-buildとCanonical Timeline public input契約を実装済み。次はsite manifest / HTML漏えい検査 |
...
3. ~~明示接続候補の初回走査完了を受け、公開用Timelineの目的・表示粒度・unknown/conflict表現を判断し、public projection schema・projector・preflight・rendererを合成fixtureで固定する。~~ 公開profileの採択は2026-09-01に完了し、Zensical dual-build / exact pinは2026-09-02、public-safe入力schema / push前review / local promotionは2026-09-04に完了。次はsite manifest / rendered HTML exposure scanを小さいPRで固定する。
