...
| M6 公開準備 | 進行中 | 公開範囲、ホスティング、更新・rollback、漏えい検査を決定し、限定公開できる | Timelineの公開profileとprojection schemaは確定。projector・preflight・renderer・公開workflowが残る |
...
3. ~~明示接続候補の初回走査完了を受け、公開用Timelineの目的・表示粒度・unknown/conflict表現を判断し、public projection schemaとfield allowlistを合成fixtureで固定する。~~ 2026-09-01に完了。次は入力不変・決定的なpure projectorとpublic-safe aggregate reportである。
