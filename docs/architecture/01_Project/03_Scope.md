...
§5.3で要求していた全キャラクター横断の部分集合性検証（dry-run）を実施した結果、**4.3.1のサンプル検証は代表的ではなく、全量では相当数の例外が確認された**（`_VR`は例外0件で確認どおりだったが、`_n`は約1割、`_spine`・`#N`は約4〜5割が部分集合関係不成立、詳細は§5.3）。**本決定（H_sceneN本体+`H_scene_s`のみパース対象、`_n`/`_VR`/`_spine`/`#N`はパース対象外）そのものは本節時点でも変更しないが、その前提（変種は本体と同一内容の変種にすぎない）は全量では成立しないことが判明したため、変種の一部を将来パース対象に含める必要があるかどうかという新たな未決事項が発生した（§5.5）**。
...
## 5.5 変種の一部をパース対象に含める必要があるか（§5.3の例外を受けた新設、2026-07-15追加、未決）

§5.3の全量検証で、`_spine`（39.4%）・`#N`（47.8%）・`_n`（10.5%）に、本体の部分集合になっていない例外が確認された（`_VR`は例外0件）。これにより、§4.3決定の前提（変種は本体と同一内容にすぎずパース対象外でよい）が全量では成立しないことが判明したため、以下がユーザー/Fable判断待ちの新規未決事項となる。**本文書・本PRでは決定しない。**

1. **`#N`（reverse_superset型が主）**: 実例確認では、本体ファイル自体が変種側の内容の一部区間（部分集合）になっているケースを含む。この場合、本体のみをパース対象とすると、変種側にしか存在しない内容が内部KBから漏れる。`#N`をパース対象に含めるか、少なくとも本体が変種の部分集合になっているケースを検出してmanifestへ記録するかを検討する必要がある。
...
3. 上記を踏まえ、(a) 現状の§4.3決定（本体+`H_scene_s`のみパース対象）を維持し例外は将来のmanifestで記録するに留めるか、(b) 例外が発生したH_sceneN単位のみ変種もパース対象に追加するか、(c) 全変種パターンをパース対象に含めるよう§4.3を改定するか、の3方向性がありうるが、いずれも本PRのスコープ外であり決定しない。
