...
### 4.3.4 全量dry-run実施済み（`feature/hscene-full-pipeline-dry-run`、2026-07-17）
...
**自動バインド方針の確定（`parser-auto-bind-non-speaker-slot-review`、2026-07-28）**:
上記実測に加え、`@SpineTalk`の2,893回中2,891回が`$numN`形式で既存の
`$numN -> slot N`契約に依存すること、話者非消費値・非ID形式値の報告分離と
`ch N`+`costume`の明示的再束縛が実装済みであることを踏まえ、
`$numX`/`$valueX`のスロット自動バインドは維持する。今後の誤帰属は
自動バインドの一律停止や数値帯による推測ではなく、意味を確認できた
コマンド固有の再束縛を時系列の後勝ちで追加して解消する。詳細な契約と
将来変更時の回帰条件は`Script_Compatibility_Check.md` §11.5を参照。
...
**2026-07-18解決**: レビューパケット6件のユーザー確認が完了し、`character-dictionary-confirmed-batch-006`で3件をconfirmed登録、残り3件は実ファイル確認によりキャラIDではなく衣装ID（`ch N`+`costume`形式の第1引数）と確定したため非登録とした（詳細は`Character_Dictionary_Review.md` §12.7参照）。
...
## 5.6 `character`カテゴリファイル種別全量調査での新発見（2026-07-16発見、`episodeN_n`系は2026-07-17決定済み）

`character-story-id-manifest-design`の設計にあたり実施した`data/raw/character/`ファイル種別全量調査（2,419件、`.gitkeep`除く）で、本文書がこれまで扱ってきたH_sceneN本体・変種・純コマンド演出ファイルの分類に含まれない種別が新たに見つかった。

- **`episodeN_n`（13件）・`episode_osawariN_start_n`（3件）**: 本編episode系（`episodeN`/純コマンド`episode_osawariN_start`）にも`_n`接尾辞の変種が存在することが判明した。§5.3の全量検証対象はH_scene変種（`H_sceneN`基準の`_n`/`_VR`/`_spine`/`#N`）のみであり、本編episode側の`_n`変種は未検証だった。2026-07-17に§5.6.1で部分集合性検証を実施し、その結果を踏まえて§5.6.2でスコープ決定済みである。
...
**示唆**: 上記の補足検証結果は、`episodeN_n`13件が実質的にH_scene系コンテンツの命名上の例外（`episode`接頭辞を持つがH_scene系本体に内容が包含される）である可能性を示している。`episode_osawariN_start_n`（3件）は例外0件・部分集合完全成立のため、H_scene変種検証における`_VR`と同様（§5.3・§5.5）に、対応本体のみを取り込み対象とし変種は一律対象外としてよい可能性が高い。この示唆を踏まえたスコープ決定は§5.6.2を参照（後続タスク`episode-n-variant-scope-decision`、2026-07-17ユーザー決定）。
...
### 5.6.2 本編episode系`_n`変種のスコープ決定（`episode-n-variant-scope-decision`、決定済み）

**決定日: 2026-07-17。以下はユーザーが明示的に決定した内容であり、数値・結論はAIエージェントが変更してはならない。** ユーザーは対象16件の実ファイルと§5.6.1の検証結果を確認した上で、以下を決定した。

1. **`episodeN_n`（13件）**: 命名は`episode`接頭辞だが実体はH_scene系コンテンツ（対応する本編episodeN本体が存在しない孤立変種であり、全13件が同番号H_sceneN本体の完全部分集合）と確定する。**H_scene系と同じtwo-tier方針（軸(A)内部KB対象・軸(B)公開恒久除外、§4.2）の対象へ編入する。** 内容がH_sceneN本体の純部分集合であるため**パース対象外**（§4.3の純部分集合変種と同じ扱い、manifest記録のみ）。懸念されていた本編公開スコープへの影響（§5.6の当初懸念）は発生しない。
2. **`episode_osawariN_start_n`（3件）**: 本体`episode_osawariN_start`の完全な部分集合（例外0件）のため、H_scene変種検証における`_VR`と同様に**一律パース対象外**とする。本体（純コマンド演出ファイル）自体の内部KB扱いは§5.1の未決バケットのまま変更しない。

実装への影響はない（docs-only決定）。manifest builder（`Character_Story_ID_Manifest_Design.md` §9のPR C）は両種別を既に`auxiliaryFiles`（`fileRole: other`）として記録しており、本決定はその既存の記録運用と整合する。`fileRole`を`variant`へ変更する実装、および動的部分集合判定（同§9のPR D）の対象拡大は、いずれも本決定の時点では行わない（両種別とも純部分集合が確定しているため判定不要。将来必要になれば別途検討する）。

詳細は`docs/architecture/05_Parser/Character_Story_ID_Manifest_Design.md` §10.1を参照。
