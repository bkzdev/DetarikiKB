...
| フェーズ | 内容 |
|---|---|
| 短期（次PR） | Story page新規生成、Episode page維持、Story index→Story pageリンクは次々PR、URL構造は候補A |
...
- Story pageを生成する（`render_story_page`相当の新規関数）
- Story page path helperを追加する（`agents/wiki_generator/paths.py`、候補A: `stories/{publicStoryId or storyId}.md`）
- Story indexからStory pageへリンクする
- Story page内にEpisode一覧を出す
- Story Summary / Episode Summaries placeholder（「未生成」）を出す
- Episode pageは残す（変更しない）
- `publicStoryId`がある場合はStory page filenameに使う
- `publicStoryId`が無い場合は`storyId`へfallback
- `publicEpisodeId`のEpisode page path方針はPR #73のまま維持（変更しない）
- `tests/fixtures/wiki/synthetic_merged_collection.json`を更新（Story page生成確認用の合成データ追加）
- `mkdocs build --strict`確認
