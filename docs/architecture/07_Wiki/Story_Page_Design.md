...
**evidenceRefs表示（`feature/story-summary-evidence-display`で実施）**: 表示可能なStory Summary/Episode Summary本文の下に、対応する`evidenceRefs`をIDのみ`Evidence refs: `ID1`, `ID2`` の1行で表示するようにした（`_render_evidence_refs_line`）。evidenceRefsが空の場合は何も表示しない（案A）。表示対象・非表示statusの扱いはSummary本文と同じ（PR #80のまま）。**Episode pageへの表示・Evidence indexへのリンク化は行っていない**。詳細は`docs/architecture/06_AI/Story_Summary_Design.md` §9参照。

**Evidence index設計（`feature/story-summary-evidence-index-design`で実施）**: `evidenceRefs`テキスト表示の将来リンク先として、Evidence indexの役割・公開範囲・Story page/Episode pageとの関係を`docs/architecture/06_AI/Evidence_Index_Design.md`で設計した。初期推奨はStory別Evidence page（`evidence/{publicStoryId or storyId}.md`）。**本PRでは設計のみ、Evidence page生成・リンク化・Story page renderer変更は行っていない**。

**Evidence index schema実装（`feature/evidence-index-schema-implementation`で実施）**: `schemas/evidence_index.schema.json`・loader（`agents/wiki_generator/evidence_index.py`）・validator（`scripts/validate_evidence_index.py`）を実装した。**Story pageのrenderer自体は変更していない**（Evidence page生成・evidenceRefsのリンク化は次PR`evidence-index-renderer-integration`）。
...
上記は初版`wiki-story-page-renderer`時点のNon-goalsの履歴である。Episode pageへのSummary/evidenceRefs表示は`episode-page-evidence-linking-review`で後続実装を推奨する決定に更新したが、general Story Evidence index link、Episode別Evidence page/episode絞込anchor、schema/storage/CLI option/path変更は引き続き対象外とする。
