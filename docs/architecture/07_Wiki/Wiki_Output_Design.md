...
**title/subtitle表示方針（設計のみ、`feature/story-manifest-design`・`feature/story-title-subtitle-import-design`。Wiki renderer実装は未着手）**: `docs/architecture/05_Parser/Story_Manifest_Design.md`で設計した`story_manifest.yaml`が将来整備された場合、Episode pageのtitle/subtitleはそこ由来の値（Normalized Story JSONの`metadata.episodeTitle`/`episodeSubtitle`経由、`Story_Metadata.md`参照）を使う想定とする。`subtitle`が設定されている場合は表示し、無い場合（`null`）は`episodeId`または「第{episodeNumber}話」のような機械的な表記でfallback表示する方針とする。AI-generated titleは、公式`title`/`subtitle`とは明確に別扱いとする（§3の情報分離方針、既存のAI analysis page区分をそのまま踏襲）。`story_manifest.yaml`の`metadataStatus: pending`由来のページでは、必要に応じて「タイトル未確認」等の注意表示を検討してよい。これらtitle/subtitleの値自体は、`Story_Manifest_Design.md` §11.7のimport candidate（`documentType: "story_title_subtitle_candidates"`）→人間レビュー→`story_manifest.yaml`反映という流れを経てから`confirmed`になる想定であり、Wiki側は反映済みの値をそのまま表示するのみで、候補の採否判定自体には関与しない。**本PRではこれらの表示ロジック自体の実装は行わない**（設計のみ）。
