...
**実装状況（`feature/episode-page-renderer-expansion`で拡張、`feature/wiki-episode-title-display-integration`でDisplay Title列を追加）**: `render_story_index_page`は、storyId・episodeId（Episode pageへのリンク）・Display Title（`displayTitle > episodeSubtitle > storyTitle > episodeId`のfallback、§9.3参照）に加え、documentId・candidate合計件数・`report.inputResults`から突き合わせたstatus（見つからない場合は空欄）を1行にまとめた表を出力する。
