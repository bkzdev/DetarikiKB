...
**evidenceRefs表示（`feature/story-summary-evidence-display`で実装）**: 上記の表示対象Summary本文の下に、`evidenceRefs`をIDのみ短く（`Evidence refs: `ID1`, `ID2``）表示するようにした。evidenceRefsが無い場合は何も表示しない。raw dialogue text/raw DEC command/raw pathは表示せず、Evidence indexへのリンク化・Episode pageへの表示はまだ行っていない。
