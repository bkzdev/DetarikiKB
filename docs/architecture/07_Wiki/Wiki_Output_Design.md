...
**title/subtitle表示方針（設計のみ、`feature/story-manifest-design`。Wiki renderer実装は未着手）**: `docs/architecture/05_Parser/Story_Manifest_Design.md`で設計した`story_manifest.yaml`が将来整備された場合、Episode pageのtitle/subtitleはそこ由来の値（Normalized Story JSONの`metadata.episodeTitle`/`episodeSubtitle`経由、`Story_Metadata.md`参照）を使う想定とする。`subtitle`が設定されている場合は表示し、無い場合（`null`）は`episodeId`または「第{episodeNumber}話」のような機械的な表記でfallback表示する方針とする。AI-generated titleは、公式`title`/`subtitle`とは明確に別扱いとする（§3の情報分離方針、既存のAI analysis page区分をそのまま踏襲）。`story_manifest.yaml`の`metadataStatus: pending`由来のページでは、必要に応じて「タイトル未確認」等の注意表示を検討してよい。**本PRではこれらの表示ロジック自体の実装は行わない**（設計のみ）。
