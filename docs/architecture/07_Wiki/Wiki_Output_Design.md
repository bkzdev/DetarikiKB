...
- **実データ初回昇格（`feature/evidence-index-promotion-first-reviewed-sample-retry`で実施）**: `knowledge/evidence/stories/`が初めて実データ由来のEvidence Index（1 story・187 entries、匿名化表記`EVT_260707_001`）を持つようになった。`render_wiki.py --evidence-index knowledge/evidence/stories`でEvidence page（`evidence/EVT_260707_001.md`）を実際に生成し、見出し・anchorが`publicEvidenceId`、内部ID非露出であることを確認した。**renderer/paths.py本体の変更は行っていない**（既存実装のまま実データに対して動作することを確認したのみ）。
