...
**Story page Related Characters改善（`codex/story-page-related-characters-refinement`で実装）**: Story内の決定的なEpisode順を基準に初出順でresolved characterを表示し、同一Characterは`canonicalId`単位で1回だけCharacter pageへリンクする。unresolvedは内部`id`単位で数えるが、Story page本文には内部ID・表示名を列挙せず`Needs Review`の件数とUnresolved reportへの導線だけを表示する。同名の別unresolved entityは統合せず、同じ内部`id`が複数Episode・複数recordに現れる場合だけ重複排除する。これにより未知情報を破棄せず、公開閲覧用一覧とreview情報を分離する。Episode pageの既存表示、Character page生成条件、Merged Knowledge schemaは変更していない。
...
**evidenceRefs表示（`feature/story-summary-evidence-display`で実装）**: 上記の表示対象Summary本文の下に、`evidenceRefs`をIDのみ短く（`Evidence refs: `ID1`, `ID2``）表示するようにした。evidenceRefsが無い場合は何も表示しない。raw dialogue text/raw DEC command/raw pathは表示せず、Episode pageへの表示はまだ行っていない。Evidence indexへのリンク化は`feature/evidence-index-renderer-integration`で実装済み（§9.16参照）。
...
**実装状況（`episode-page-summary-evidence-linking`で実施）**: Episode pageへ対象Episodeの表示可能なEpisode Summary本文と、その直下の`evidenceRefs`だけを追加した。表示は既存Story pageと同じ`generationStatus: generated`、`review.status: reviewed`/`approved`、内部/公開ID照合矛盾時は非表示という契約に従う。Summaryが欠落・非表示・空本文なら`## Episode Summary` sectionを出さず、既存出力を維持する。Story Summaryは再掲しない。参照は既存Story別Evidence pageの該当anchorへ公開ID優先でリンクし、未解決時は入力IDをbacktickで表示する。general Story Evidence index link、Episode別Evidence page/episode絞込anchor、schema/storage/CLI option/path変更は含めていない。local manual reviewでは通常幅・390×844狭幅とも表示崩れ・水平overflow・browser console errorが無く、Evidence anchor遷移も内部/公開ID系双方で正常だったため、現時点で追加導線は不要と判断した。
...
**実装状況（`codex/wiki-location-page`）**: `render_location_page`と
`location_page_path`を追加し、`canonicalId`確定かつ`status: merged|conflict`で
§6の防御条件を満たす場所だけを
`locations/{canonicalId}.md`へ生成する。Summary、Aliases、`sceneRefs`件数、
Evidence、Source Candidates、ConflictsをCharacter pageと同じ安全な要約形式で
表示する。登場Episodeは`evidenceRefs[].episodeId`、
`sourceCandidates[].episodeId`、`extractionRunRefs`のキーから重複排除して算出し、
...
- **renderer統合（`feature/evidence-index-renderer-integration`で追加）**: `scripts/render_wiki.py --evidence-index <path>`を追加し、`render_evidence_page`でStory別Evidence page（`evidence/{publicStoryId or storyId}.md`、1 story 1 page）を生成するようにした。表示項目はType/Episode ID/Public Episode ID/Scene ID/Block ID/Speaker/Related Entities/Referenced byのみ（raw text/raw DEC command/raw path/generatedFrom詳細/inputRefs詳細/extraction JSON dump/normalized block dump/prompt/LLM生出力/workspace pathはいずれも表示しない、§4の非表示方針の踏襲）。Story page「Review Links」section・Story/Episode Summaryの`evidenceRefs`は、該当storyのEvidence Indexが存在する場合のみリンク化する（Story_Page_Design.md/Story_Summary_Design.md参照）。Evidence Indexの検証（schema + raw text禁止文字列 + `rawTextIncluded`）は`--validate`フラグの有無に関わらず常に実行する（安全性優先）。**Evidence Index自動生成（Normalized Story JSON/Extraction Resultから）・Internal Review Evidence Packet生成・Evidence top page（`evidence/index.md`）・Episode pageへの変更は行っていない**（次PR候補`evidence-index-generation-dry-run`/`internal-review-evidence-packet-design`）。
- **候補生成dry-run（`feature/evidence-index-generation-dry-run`で追加）**: `scripts/build_evidence_index_candidates.py`でNormalized Story JSON/Extraction ResultからPublic Evidence Index候補YAMLを`workspace/evidence_index_dry_runs/`配下にdry-run生成できるようにした（`docs/runbooks/Evidence_Index_Generation_Dry_Run.md`）。**`knowledge/evidence/stories/`への自動昇格は行っていない**。
- **昇格方針レビュー（`feature/evidence-index-generation-review`で追加）**: 実データdry-run結果（`stage_direction`がentryの大半を占める傾向）を踏まえ、Public Evidence Indexの初期公開対象entry type・`knowledge/evidence/stories/`への昇格条件（promotion criteria）・除外条件・Evidence page size policyを`docs/architecture/06_AI/Evidence_Index_Promotion_Policy.md`にまとめた。**promotion script実装・filter機能実装・実Evidence Indexのcommitは行っていない**（次PR候補`evidence-index-generation-filtering`/`evidence-index-promotion-policy-implementation`）。
- **entry type filtering（`feature/evidence-index-generation-filtering`で追加）**: `scripts/build_evidence_index_candidates.py`に`--public-profile default|full|review`（デフォルト`default`）・`--include-types`・`--exclude-types`を追加し、Public向けdry-run候補生成では`stage_direction`をデフォルトで除外するようにした。PR #85と同じ匿名化サンプルでentry数が1793件→187件に縮小することを確認した。**Evidence page renderer自体は変更していない**（`render_wiki.py --evidence-index`は引き続きfilter後のYAMLをそのままrenderするのみ）。
- **promotion check（`feature/evidence-index-promotion-policy-implementation`で追加）**: `scripts/check_evidence_index_promotion.py`を追加し、Evidence Index候補が`knowledge/evidence/stories/`へ昇格可能かを機械的にcheckできるようにした（`docs/runbooks/Evidence_Index_Promotion_Check.md`）。schema検証・raw text scan・`--policy public-default`のentry type policy check・`--story-summaries`指定時のSummary evidenceRefs整合性確認（missingはwarning）を実装。**check-onlyであり、実際のcopy・commit・自動昇格・Evidence page renderer変更は行っていない**（次候補`evidence-index-promotion-dry-run`/`evidence-index-promotion-copy-script`）。
- **generation dry-run（`feature/evidence-index-generation-dry-run`で追加）**: `scripts/build_evidence_index_candidates.py`でNormalized Story JSON（任意でExtraction Resultも）からPublic Evidence Index候補YAMLをdry-run生成できるようにした（`docs/runbooks/Evidence_Index_Generation_Dry_Run.md`）。生成物は`workspace/evidence_index_dry_runs/`（`.gitignore`済み）配下のみで、`knowledge/evidence/stories/`への自動昇格は行わない。詳細は`Evidence_Index_Design.md` §10参照。
- **publicEvidenceId中心へのrenderer切替（`feature/evidence-index-public-id-renderer-switch`で追加）**: `render_evidence_page`のentry見出し（`### {evidenceId}`）と`_evidence_ref_link`（Summary `evidenceRefs`リンクの表示テキスト・anchor）を、`publicEvidenceId`優先（無ければ内部`evidenceId`にfallback）へ切り替えた（`agents/wiki_generator/evidence_index.py`の`display_evidence_id`/`resolve_evidence_entry`）。Evidence page URL自体は既存どおり`publicStoryId`優先のまま変更していない。Public-safe projection output（内部`storyId`が`publicStoryId`の値へ置換される）をrenderする際、Story pageの「Review Links → Evidence index」導線が内部`storyId`だけでは解決できない問題を発見し、`resolve_story_evidence_entries`（内部`storyId`→`publicStoryId`の順でfallback）で対応した。匿名化実データサンプル（Public-safe projection、187 entries）でrender確認し、Evidence page内の内部ID非露出を確認した。**Evidence Index schema変更・Episode page変更・実Evidence Indexのcommitは行っていない**（次候補`evidence-index-promotion-first-reviewed-sample-retry`）。
- **実データ初回昇格（`feature/evidence-index-promotion-first-reviewed-sample-retry`で実施）**: `knowledge/evidence/stories/`が初めて実データ由来のEvidence Index（1 story・187 entries、匿名化表記`EVENT_164_260425`）を持つようになった。`render_wiki.py --evidence-index knowledge/evidence/stories`でEvidence page（`evidence/EVENT_164_260425.md`）を実際に生成し、見出し・anchorが`publicEvidenceId`、内部ID非露出であることを確認した。**renderer/paths.py本体の変更は行っていない**（既存実装のまま実データに対して動作することを確認したのみ）。
...
9. ~~**episode-page-summary-evidence-linking**~~: 対象Episodeの表示可能なEpisode Summary本文と、その直下の`evidenceRefs`をEpisode pageへ限定追加し、合成fixture検証とlocal manual reviewまで完了した。Episode別Evidence page等の追加拡張は現時点で不要と判断した。
