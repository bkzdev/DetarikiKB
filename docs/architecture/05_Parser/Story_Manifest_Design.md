...
## 13.2 public ID fields（`feature/story-manifest-public-id-fields-design`で追加）

`Story_ID_Policy_Decision.md` §7で採用決定した、公開Wiki URL用の任意フィールド`publicStoryId`（story-level）/`publicEpisodeId`（episode-level）を追加した。
...
- **既存manifestとの後方互換**: `publicStoryId`/`publicEpisodeId`を含まない既存manifest（`docs/templates/story_manifest_template.yaml`含む、両フィールドとも`null`で追加済み）はそのままschema検証を通過する（追加フィールドのみ、既存フィールドの意味・必須性は変更していない）
- **category別の例（すべて合成データ）**:
...
## 13.3 characterId / auxiliaryFiles（`feature/story-manifest-character-category-support`で実装完了）

`Character_Story_ID_Manifest_Design.md` §8.1で設計した、CHARACTERカテゴリのstory専用の任意フィールドを`schemas/story_manifest.schema.json`へ追加した（PR C）。

```yaml
storyId: "CHAR_HS_AKAGI_HINA"
characterId: "CHAR_AKAGI_HINA"
category: "character"
auxiliaryFiles:
  - rawPath: "CHARACTER/csl_script_charastory_character990102_export/CAB-csl_script_charastory_character990102-H_scene6_n.dec"
    sourceFileName: "CAB-csl_script_charastory_character990102-H_scene6_n.dec"
    fileRole: "variant"
    notes: null
episodes: [ ... ]
```

- **`characterId`**: story-level任意フィールド、`oneOf: [パターン^CHAR_[A-Z0-9_-]+$, null]`。当該storyが紐づくキャラクター（`knowledge/dictionaries/characters.yaml`のconfirmed `characterId`）を記録する。CHARACTERカテゴリのstoryのみ設定する想定で、それ以外はnullまたは省略する
- **`auxiliaryFiles`**: story-level任意フィールド、配列（空配列も有効）。各要素は`rawPath`/`sourceFileName`/`fileRole`（`variant`/`direction`/`other`のenum）/`notes`（null許容）を持つ。H_scene変種・camera/finish等の演出コマンド専用ファイル・img/test等の特殊ファイルを、独立episodeとしてはパースせずstory単位で記録する（詳細な分類方針は`Character_Story_ID_Manifest_Design.md` §8.1参照）
- **後方互換**: 両フィールドとも既存manifestには存在しないため、既存の`docs/templates/story_manifest_template.yaml`（EVENT例）はそのままschema検証を通過する。テンプレートには合成キャラクター`CHAR_AKAGI_HINA`を使ったCHAR_HS story例（`characterId`/`auxiliaryFiles`各fileRoleの使用例を含む）を追加した
...
## 16.1 CHARACTER / CHARACTER_DATE対応（`feature/story-manifest-character-category-support`で実装完了）

`Character_Story_ID_Manifest_Design.md` §4・§8・§9のPR Cとして、`CHARACTER`・`CHARACTER_DATE`カテゴリ対応を実装した。コアロジックは`agents/parser/story_manifest_candidates.py`の`build_character_story_manifest_candidates()`（CLI側は`scripts/build_story_manifest_candidates.py`の`--character-dictionary`引数、既定値`knowledge/dictionaries/characters.yaml`）。

- 対応するraw配置: `CHARACTER/csl_script_charastory_character{N}_export/CAB-csl_script_charastory_character{N}-{suffix}.dec`・`CHARACTER_DATE/csl_script_surprise_character{N}_export/CAB-csl_script_surprise_character{N}-Surprise_{M}.dec`（大文字小文字を区別しない、EVENTと同じ方針）
- `{N}`（sourceCharacterId）は`knowledge/dictionaries/characters.yaml`でconfirmed化されている場合のみcandidate生成対象とする（`agents/parser/character_dictionary.py`の`load_character_dictionary`/`resolve_character_by_source_id`を再利用）。未confirmedのキャラクターはcandidate生成対象外とし、CLI標準出力へpending報告として表示する（黙って除外しない）
...
**解消（`feature/character-story-id-manifest-design`で2026-07-16決定により解消）**: `data/raw/character/`・`data/raw/character_date/`の実配置全量調査（72キャラクター分）を踏まえ、storyId体系（ローマ字characterId方式、新設カテゴリ`CHAR_HS`を含む）とprefix判定ロジック（raw配置ディレクトリ＋ファイル名サフィックスの組み合わせ）を`docs/architecture/05_Parser/Character_Story_ID_Manifest_Design.md`で確定した。**このPRでも候補生成scriptの実装自体は行っていない**（実装はPR C、同文書§9参照）。
