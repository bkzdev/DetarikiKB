...
| `CHARACTER`（想定） | `character` | `CHAR_MAIN`/`CHAR_EXTRA`/`CHAR_DATE`/`CHAR_HS`のいずれか | **対応方針確定**（`docs/architecture/05_Parser/Character_Story_ID_Manifest_Design.md`参照、2026-07-16ユーザー決定。実装はPR C）。raw配置ディレクトリ（`character`/`character_date`）とファイル名サフィックスの組み合わせでprefixを機械判定する |
