...
**実装状況（Speaker Label Normalization設計、`agents/parser/speaker_labels.py`）**: `name`コマンド/`@ChTalkName`由来のspeaker label（キャラクターID経由ではなく表示名として直接指定された話者名）は、未登録キャラクターID由来の`不明人物(ID:xxx)`とは区別する。`speaker`へ任意フィールド`labelSource`（`name_command`/`ch_talk_name`）と`labelAnalysis`（構造化結果）を追加する。
...
`labelType`は`single_speaker`/`speaker_group`/`speaker_with_modifier`/`speaker_group_with_modifier`/`generic_speaker`/`ambiguous_speaker`/`unknown`のいずれか。通常の単独キャラクター名（`single_speaker`）は特殊扱いしない。`resolutionStatus`は`not_applicable`/`needs_review`/`inferred`のいずれかのみが自動で付与され、**`confirmed`は人間レビュー結果取り込み用に予約するのみで自動では付与しない**。`inferredSpeakers`はconfirmed character dictionaryとの参考照合結果であり、これがあっても`speakerId`/`isResolved`は変更しない（自動でのcharacter確定はしない）。詳細は`Extraction_Result_Schema.md`の`SpecialSpeakerLabelCandidate`を参照。
