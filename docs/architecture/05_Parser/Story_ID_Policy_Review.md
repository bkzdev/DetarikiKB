...
**実装状況（`feature/story-id-policy-design-decision`で追加）**: 本レビューの比較結果を踏まえ、DKBが採用するID方針を`docs/architecture/05_Parser/Story_ID_Policy_Decision.md`で正式に決定した。採用方針（既存storyId/episodeIdは当面維持、public ID分離は`publicStoryId`/`publicEpisodeId`として次PRで設計）・category別方針・採用しない案・migration方針（additive first）を確定した。**このPRでもID生成ロジック・schema・URL/file pathは変更していない。**
