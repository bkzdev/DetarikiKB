...
**採用文言**: Public Evidence Indexは、公開repoに保存する場合、`publicStoryId`/`publicEpisodeId`/`publicEvidenceId`を中心にしたprojectionとして保存する。内部trace ID（sourceKey由来の`storyId`/`episodeId`/`blockId`等）は、`workspace/review_packets/evidence/`側のInternal Review Evidence Packetにのみ置く。Packetは詳細設計済み・実装未着手である（`Internal_Review_Evidence_Packet_Design.md`）。
...
- **mapping output commit禁止**: `--mapping-output`のCSVは内部ID（`storyId`/`episodeId`/`evidenceId`等）と公開IDを1行に並べて記録するため、常にworkspace配下に留め、**commit禁止**とする。Internal Review Evidence Packetの正式なmapping componentとして取り込む設計とした（`Internal_Review_Evidence_Packet_Design.md` §8、generatorは未実装）
...
- **mapping output**: `--mapping-output`はpublic-safe modeでも内部ID⇔公開IDのmappingを引き続き出力する（compatible modeと同じ列構成）。Internal Review Evidence Packetの`mappings/evidence-id-map.csv`へ同じprojection作業の組として渡し、candidate/Normalized Storyとのcross-reference整合を検証して取り込む設計であり、**commit禁止**は不変
...
| D | `workspace/review_packets/evidence/`（Internal Review Evidence Packet、設計済み・実装未着手）にのみ保持する |

## 7.2 推奨

- 公開repoに保存するPublic Evidence Indexでは、sourceKey由来の内部IDを**そのまま大量に保持しない**（候補Aに近い方向）
- 完全除外後の照合は、`workspace/review_packets/evidence/<packetId>/mappings/evidence-id-map.csv`に置くmapping tableで行う（候補Dを採用）。`packetId`とbundle内ordinalはsourceKey/内部IDから作らない
- 「Public Evidence Index本体には内部IDを持たせない」「照合が必要な場合はInternal Review Evidence Packet側のmapping tableを参照する」という役割分担を採用する
- mapping tableは`project_evidence_index_public_ids.py --mapping-output`と同じ列を使い、candidate/Normalized Storyとcross-consistentな入力組として取り込む。既定14日・最大30日のephemeral retention、commit禁止、明示cleanupの対象とする。詳細は`Internal_Review_Evidence_Packet_Design.md` §5・§6・§8・§12を正とする
...
- Internal Review Evidence Packet用にはmanifest/story/selectionの専用schemaと専用validatorを別途実装する（詳細設計済み・実装未着手）。Public Evidence Index schemaとは混在させない（`Internal_Review_Evidence_Packet_Design.md` §4・§7）
...
| Phase 6: `internal-review-evidence-packet-design` | 内部trace ID・mapping tableをInternal Review Evidence Packet側で扱う詳細設計 | **完了（設計のみ）** |
...
- Internal Review Evidence Packet生成（mapping CSVの出力自体は実装済み、Packetの設計・保管場所は`internal-review-evidence-packet-design`で確定済みだが、schema/generatorは未実装）
...
- ~~`internalTrace`/mapping tableの生成方法~~ → **`internal-review-evidence-packet-design`で解決**: 既存`project_evidence_index_public_ids.py --mapping-output`をcandidate/Normalized Storyとcross-reference検証して取り込み、Packet generator側では再採番しない
- ~~mapping tableの保管場所・アクセス制御~~ → **`internal-review-evidence-packet-design`で解決**: `workspace/review_packets/evidence/<packetId>/mappings/evidence-id-map.csv`、commit禁止、local-only、ephemeral retentionとする
...
- ~~`episodeOrder`の正式な根拠とepisode追加・順序変更時の通常運用~~ → **`public-id-manifest-assignment-policy`で解決**: 人間確認済み`story_manifest.yaml`の`episodeNumber`を正式な順序根拠とし、entry初出順は候補用ヒューリスティックに限定する。未公開は全候補を再レビューし、公開済みの途中挿入・順序変更は専用migration設計なしに実行しない（`Public_ID_Registry_Design.md` §8、`docs/runbooks/Public_ID_Manifest_Assignment.md`）
...
- `docs/architecture/06_AI/Internal_Review_Evidence_Packet_Design.md`（内部ID⇔公開ID mappingのbundle配置・cardinality・保持・削除方針）
- `TASKS.md`（次PR候補の追跡）
...
- ~~RAIDカテゴリの採番方式は`§16.7.4`のopen questionとして新たに据え置く（v2.1はEVENTカテゴリのみを対象に全体採番を確定した）~~ → **§16.8（2026-07-15ユーザー決定）で解決**: RAIDカテゴリにもv2.1の全体採番方式を適用し、既公開RAID 1 storyを再改名した
...
→ **§16.9（2026-07-18ユーザー決定）で改定**: 末尾seq方式（適用実績ゼロ）は、「publicStoryIdをID文字列としてソートしたとき実際の時系列位置に並ぶこと」を要件とする**アンカーseq+`_ADD`マーカー方式**へ置き換えられた。以後、遅延発見storyの採番は§16.9を正とする。

### 16.7.4 RAIDカテゴリの扱い（Open question→§16.8で解決）

- 既公開RAID 1 story（`publicStoryId`は本節では匿名化のため記載しない、§16.7.5参照）は、**本PRでは変更しない**
- ~~RAIDカテゴリの採番方式（v2のRegistry登録順を維持するか、v2.1と同じ全体採番方式に揃えるか）は、raid batchへ着手するタイミングで別途設計する。本PR時点では未確定のまま据え置く~~ → **§16.8（2026-07-15ユーザー決定）で解決**: v2.1と同じ全体採番方式に揃え、既公開RAID 1 storyを再改名した
...
- RAIDカテゴリの既公開1 story（`RAID_001_260504`）は§16.7.4のとおり本PRでは変更しない（のちに§16.8でRAIDカテゴリにもv2.1を適用し`RAID_027_260504`へ改名、新旧mappingは§16.8.2参照）
...
- `RAID_001_260504`の変更（§16.7.4、のちに§16.8で実施、詳細は§16.8.2参照）
- raidカテゴリの採番方式の設計（§16.7.4、のちに§16.8で実施）
- Stage 2候補（新規5 story）の昇格
- `scripts/`本体の変更

## 16.8 RAIDカテゴリへのv2.1適用（2026-07-15ユーザー決定、`feature/raid-public-id-v2-1-alignment`で設計・実行）

**本節はユーザー決定を確定したものであり、覆さない。本PRは設計（本節の追記）と実行（既公開RAID 1 storyの再改名）を同一PRで行った。§16.7.4・§16.6で据え置いたRAIDカテゴリのopen questionを解決する。**

### 16.8.1 対象母集団と採番規則（決定）

対象母集団は`data/raw/raid/`配下のraid category全量（27 export dir）とする。EVENTカテゴリと異なり、全件が名前中に一意の6桁日付を持つ（日付接頭辞を持たない旧番号体系のsourceKeyは存在しない）。

1. **YYYYMM形式（月精度、4件）**: YYYYMM昇順でseq `001`〜`004`を割り当てる。publicStoryId形式は`RAID_{seq:03d}`（**日付サフィックスなし**）。これはEVENTの旧形式ルール（§16.7.2の1.）と同型であり、「サフィックスは常にYYMMDD（6桁日精度）」という不変則を維持するための扱いである
2. **YYMMDD形式（23件）**: 日付昇順でseq `005`〜`027`を割り当てる。publicStoryId形式は`RAID_{seq:03d}_{YYMMDD}`（v2と同じ、日付サフィックスあり）
3. 母集団の時系列は完全分離しており（YYYYMM形式4件すべてがYYMMDD形式23件より古い）、上記2グループの境界はそのまま日付順序と一致する
4. **遅延発見イベントのルール**: §16.7.3のルールをそのままRAIDカテゴリにも適用する（末尾seq付与、既存storyのseq繰り下げは行わない）。**§16.7.3は2026-07-18に改定済み（アンカーseq+`_ADD`マーカー方式）であり、RAIDカテゴリにも改定後ルールが適用される**（詳細は§16.9参照）
5. **全量採番表**: `workspace/local_inputs/raid_numbering_table.tsv`（Fable生成済み、非commit）を正とする。カテゴリ全量のpublicStoryId ⇔ sourceKeyの対応を1行1storyで保持する

`publicEpisodeId`の形式`{publicStoryId}_E{NN}`、`publicEvidenceId`の形式`{publicEpisodeId}_{PREFIX}{sequence:04d}`はいずれも**不変**。v2.1適用で変わるのは`publicStoryId`本体の`seq`の根拠（Registry登録順→事前確定した全体番号）のみである。

### 16.8.2 再改名対象（既公開RAID 1 story）と新旧mapping

v2（§16.2〜§16.6）で確定した既公開RAID 1 storyのpublicStoryIdを、v2.1の全体採番方式に基づき再改名した。

| # | カテゴリ | 旧publicStoryId（v2、Registry登録順ベース、廃止） | 新publicStoryId（v2.1、全体採番方式） |
|---|---|---|---|
| 1 | RAID | `RAID_001_260504` | `RAID_027_260504` |

- 新publicStoryIdの`YYMMDD`部分（`260504`）は、v2の値からそのまま変わらない（sourceKeyの日付接頭辞自体は不変のため）。変わるのは`seq`部分（`001`→`027`）のみである
- `seq`値は`workspace/local_inputs/raid_numbering_table.tsv`（§16.8.1の5.）における全体番号をそのまま用いた

### 16.8.3 旧v2 ID廃止と再利用禁止

- §16.8.2の旧publicStoryId（`RAID_001_260504`）は、本PRでの再改名実行後は`knowledge/public_ids/story_public_ids.yaml`から削除される。**削除後もこのID値は将来にわたって再利用しない**（§16.4・§16.7.6と同じ安定性原則を適用する）
- v2形式（Registry登録順ベースの`seq`）は、§16.7.6でEVENTカテゴリについて廃止したのと同じく、RAIDカテゴリでも廃止する。以後、RAIDカテゴリの新規Registry entryの追加にもv2.1（全体採番方式）の`seq`を使う

### 16.8.4 本PRのNon-goals

- raid batch昇格（新規storyのRegistry追加・Evidence Index昇格）そのもの
- EVENTカテゴリ・MAINカテゴリ・OTHERカテゴリの変更
- `scripts/`本体の変更

## 16.9 遅延発見イベントルールの改定（アンカーseq+ADDマーカー方式、2026-07-18ユーザー決定、`feature/late-discovery-anchor-seq-add-rule`で設計・適用）

**本節はユーザー決定を確定したものであり、覆さない。本PRは設計（本節の追記）と適用（event categoryの遅延発見6件へのpublicStoryId確定）を同一PRで行った。§16.7.3で定めた末尾seq方式を置き換える。**

### 16.9.1 改定理由

- event categoryのraw配置に投入漏れ6件（複数年帯にまたがる）が新たに発見された。§16.7.3の末尾seq方式をそのまま適用すると、これら6件は確定済みの全体番号列の末尾に追加され、日付順序が崩れる（末尾seqのstoryが、より若いseqのstoryより古い日付を持ちうる）
- ユーザーは「`publicStoryId`をID文字列としてソートしたとき、実際の時系列位置に並ぶこと」を要件として明示決定し（2026-07-18）、末尾seq方式に代わる新方式を採用した
- §16.7.3の末尾seq方式は、本改定時点まで適用実績がゼロだったため、既存の`seq`割当・既存publicStoryIdへの影響なく置き換えられる

### 16.9.2 アンカーseq+ADDマーカー方式（決定）

1. 遅延発見storyには、既存storyの`seq`を一切変更せず、**アンカーseq**（当該storyの日付以前で最新の既存storyのseq）を重複付与する
2. publicStoryId形式は`{CATEGORY}_{アンカーseq:03d}_{YYMMDD}_ADD`とする（例: `EVENT_070_990101_ADD`のような合成例。実際の適用ID・日付・sourceKeyは本文書に記載しない）
3. `_ADD`マーカーは遅延挿入分であることを明示する。固定長`YYMMDD`が挿入位置のソートを担うため、ASCII文字列ソートで「アンカー→挿入分→次seq」という時系列順が保たれる
4. 同一アンカーseq・同一日付の衝突が万一発生した場合は、`_ADD2`・`_ADD3`のように連番を拡張する
5. `seq`単独ではstoryを一意に特定できなくなる（同一`seq`値を複数storyが共有しうる）が、`publicStoryId`全文字列としての一意性は維持される。Registry・URL・全schema patternはいずれも`publicStoryId`全文字列を単位として扱うため、この変更による影響はない

### 16.9.3 §16.7.3（末尾seq方式）の廃止

- §16.7.3で定めた末尾seq方式は、本節（16.9.2）の方式へ置き換えられ、**廃止**する。§16.7.3は適用実績ゼロのまま廃止されるため、既存publicStoryIdの改名・旧ID廃止処理は発生しない
- §16.8.1の4.（RAIDカテゴリへの§16.7.3参照）は、本改定後のルール（アンカーseq+`_ADD`マーカー方式）を指すものとして扱う

### 16.9.4 初回適用（event categoryの遅延発見6件）

- 2026-07-18、event categoryの遅延発見6件へ本方式を適用し、publicStoryIdを確定した。event categoryの母集団は168件→174件（§16.7.2の168件+遅延発見6件）に拡大した
- 具体的なpublicStoryId・日付・sourceKeyは`workspace/local_inputs/event_numbering_table.tsv`（非commit）を正とし、本文書には件数・方式のみを記録する（`AI_PR_Playbook.md` §5の匿名化ルールに従う）
- Registry（`knowledge/public_ids/story_public_ids.yaml`）への追加（promotion）は本PRでは行わない。既存運用（昇格まで行わない）のまま据え置く

### 16.9.5 本PRのNon-goals

- 遅延発見6件のraw配置・normalize・compatibility check（ユーザー配置後の別作業）
- Registry（`knowledge/public_ids/story_public_ids.yaml`）への追加
- `scripts/`本体の変更
