...
| Phase 4.13: `evidence-index-public-id-projection`（本PR） | Compatible projection（案A）の実装。`scripts/project_evidence_index_public_ids.py`で`publicEvidenceId`を実際に生成・付与する（内部IDは削除しない） | **完了（本PR、Public-safe projection・renderer切替・promotion再開は未着手）** |
