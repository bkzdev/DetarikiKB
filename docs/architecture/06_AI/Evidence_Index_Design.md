...
| Phase 4.14: `evidence-index-public-id-public-safe-projection`（本PR） | Public-safe projection（案B）の実装。`scripts/project_evidence_index_public_ids.py`に`--projection-mode public-safe`を追加し、内部ID（`evidenceId`/`storyId`/`episodeId`/`sceneId`/`blockId`）を公開ID中心へ置換・除去する | **完了（本PR、renderer切替・実promotion再開・publicEpisodeId自動補完は未着手）** |
