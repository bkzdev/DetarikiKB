...
Stage BはTimelineをcanonical化せず、`timeline/timeline_entries.json`へ観測を集約する。`sourceTimelineId`が無いScene候補はcandidate単位のまま保持する。複数Stage A documentの`relative_order`は、merge前のcandidate provenanceを失わない独立check `scripts/check_timeline_consistency.py`で検査する。`same_time`を同値classへ縮約し、class内before/after矛盾とclass間循環を検出する。順序の確定と数値順序の総合判定は引き続き将来のTimeline Builderへ委ねる（`docs/runbooks/Timeline_Consistency_Check.md`）。
