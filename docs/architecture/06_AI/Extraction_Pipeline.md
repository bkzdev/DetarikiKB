...
# 8. ローカルLLM / OpenAI API / 手動補正の使い分け
...
`extractionRun.extractionMethod: "local_llm"` として記録する。

## 8.2 OpenAI API（補助用途のみ）

対象:

- ローカルLLMで精度不足が確認された特定タスク（複雑な関係性推論、長距離文脈が必要な矛盾検出など）
- 品質検証用のサンプル比較（同一エピソードをローカルLLMとOpenAI APIの両方で抽出し、精度差を確認する用途）

`extractionRun.extractionMethod: "openai_api"` として記録し、通常運用のデフォルトにはしない。APIキーは `AI_CONTEXT.md` 13.5 の通り `.env` / 環境変数で管理し、リポジトリに含めない。
...
- 通常の抽出処理はローカルLLMを既定とし、OpenAI APIは補助用途に限定する
