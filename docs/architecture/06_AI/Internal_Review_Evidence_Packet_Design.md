...
- Packet schema、validator、loader、generator、cleanup CLIの実装
...
bundle形式のため、後続実装では少なくとも次をPublic Evidence Indexとは別に用意する。
...
Public Evidence Indexの`FORBIDDEN_TEXT_PATTERNS`はPacket validatorへ流用しない。Packetではraw内容が正当に存在し得るためである。代わりに、raw fieldの配置を`rawContent`/`context`へ限定し、safe componentへの混入、絶対path、未知field、bundle外参照を検査する。
...
Packet generatorは公開IDを再採番せず、operatorが同じprojection作業の組として渡したmapping/candidate/reportをcross-reference検証してbundleへ取り込む。現在はrun identity自体を証明できないため、§6.2のcross-consistent snapshotを保証範囲とする。これにより、projection scriptとPacket側に採番ロジックを二重実装しない。
...
## 10.1 出力先のfail-closed check

後続generator/validatorは、書き込み前に次をすべて確認する。
...
- Public-safe candidateとmappingの公開ID一致
- projection mode/policyと、mapping/Public-safe candidate/Registry digestの一致
...
validation失敗時はfinal bundleを作らない。一時directoryを削除し、stdout/stderrにはsafe aggregateだけを出す。一時directory削除にも失敗した場合は、固定root内のopaqueな`.tmp-<packetId>`相対pathだけを示してnon-zero終了する。
...
| Phase 5.2: `internal-review-evidence-packet-schema-validator` | 専用manifest/story/selection schema、合成fixture、validator、safe report | 未着手 |
...
Phase 5.2〜5.4は分離PRとする。Phase 5.2では合成fixtureだけを使い、Phase 5.3以降の実データ確認も`workspace/`限定・非commitとする。review note取り込みや自動promotionはこれらのphaseへ便乗させない。
