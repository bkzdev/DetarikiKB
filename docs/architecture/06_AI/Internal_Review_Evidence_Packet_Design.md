...
Version: 0.4
Status: Phase 5.1設計確定・Phase 5.2 schema/validator・Phase 5.3 generator・Phase 5.4 operations実装済み
...
Phase 5.3でPacket generator、Phase 5.4でcleanup CLI/inventory/runbookを実装したが、実データPacket生成、context opt-in、review note取り込み・自動promotionは引き続き範囲外とする。
...
`.gitignore`は補助防壁であり、安全性の根拠を`.gitignore`だけに置かない。Phase 5.3 generatorはroot固定、Git ignore確認、tracked file確認、symlink/junction確認を実行時に強制する（§10）。
...
- 同じprojection作業の組として渡すPublic-safe candidate: 公開側との照合用（過去run identityではなく§6.2のcross-reference整合を検証する）
...
現行projection reportはMarkdownのみで、入力schema・digest欄・run identityを持たない。このためPhase 5.3のgeneratorはprojection reportを機械入力にしない。Normalized Story、Public-safe candidate、mappingのcross-referenceを機械保証し、同じprojection作業の組を渡すことはoperator要件として残す。structured projection reportが導入されるまで、同一process invocationを証明する機能は提供しない。

Phase 5.3 CLIは次の契約で実装する。`--normalized-input`、`--public-candidate`、`--projection-mapping`は必須、`--extractions`、`--registry`、`--selection-file`、`--retention-days`は任意とする。`packetId`とoutput pathはgeneratorが固定root内で生成・決定し、CLI引数では受け取らない。

```bash
uv run python scripts/generate_internal_review_evidence_packet.py \
  --normalized-input <file-or-directory> \
  --public-candidate <file-or-directory> \
  --projection-mapping <mapping.csv> \
  [--extractions <file-or-directory>] [--registry <registry.yaml>] \
  [--selection-file <basename.json>] [--retention-days 14]
```
...
複数file入力（Normalized Story、Extraction、Public-safe candidate）のaggregate digestは、各file bytesのSHA-256 **binary digest**をbyte順でsortし、連結したbytesをSHA-256して得る。単一file入力のprojection mappingとRegistryは、そのfile bytesのSHA-256をそのまま使う。file pathやfile nameはdigest入力へ含めない。

Normalized Story、mapping、Public-safe candidateのID対応・対象entry・digestが相互に整合しない場合は生成を失敗させる。同一storyの異なるepisodeを別Normalized Story JSONとして渡すことは許可するが、`(storyId, episodeId)`の重複と、同じ`storyId`での`publicStoryId`不一致はblockingとする。`publicEpisodeIdSource=registry`のmappingでは、Normalized Story側の`publicEpisodeId`が未設定であることと、mappingの公開Episode IDが指定Registryの同じ`publicStoryId`/`episodeOrder`に一致することを検証する。Registry補完後の値をNormalized Story側にも存在すると誤認して要求しない。既存projection出力には共通run IDや親digestがないため、v1が機械的に保証するのは**cross-reference整合**であり、「同一process invocationだったこと」の証明ではない。同じprojection作業で得たmapping/candidateを組として渡すこと自体はoperator要件とする。将来structured projection reportへ相互digestを埋め込むまで、文書上も「同一runを証明」とは扱わない。
...
- `public-candidate`ではPublic-safe candidateの全entryを、公開ID三組と`evidenceType`でmappingへ1対1に解決して選ぶ。`explicit-entry-list`ではselection fileの`evidenceIds`をmappingへ1対1に解決して選ぶ。いずれもIDの類似・title・順序から補完しない
- selection fileの`evidenceId`がmapping内で複数行に一致する場合もblockingとする。schemaが単体IDのglobal uniquenessを表現しないため、generatorが曖昧性を受理しない
...
validatorは固定root配下の既存bundleを**read-only**で検査する。`reports/validation.json`はPhase 5.3 generatorがsafeな集計として作成するcomponentであり、validator自身は作成・更新しない。Public Evidence Indexの`FORBIDDEN_TEXT_PATTERNS`はPacket validatorへ流用しない。Packetではraw内容が正当に存在し得るためである。代わりに、raw fieldの配置を`rawContent`/`context`へ限定し、safe componentへの混入、絶対path、未知field、bundle外参照を検査する。
...
Packet generatorは公開IDを再採番せず、operatorが同じprojection作業の組として渡したmapping/candidateをcross-reference検証してbundleへ取り込む。projection reportは§6.1の理由で機械入力にしない。現在はrun identity自体を証明できないため、§6.2のcross-consistent snapshotを保証範囲とする。これにより、projection scriptとPacket側に採番ロジックを二重実装しない。Phase 5.2 validatorの責務は、生成済みbundleのschema・digest・mapping・story component間の自己整合性である。外部のsource candidate、Normalized Story、Registry入力とのcross-checkは、入力を読むPhase 5.3 generatorの責務とする。
...
Phase 5.3 generatorは本文を`text`、`rawText`、`choiceText`の順で最初に得られた文字列だけから採る。`rawCommand`と`rawContent.arguments`は、Normalized blockに`rawCommand`とstring配列の`args`があり、`parser-diagnostic`として許可できる場合だけ保持する。`args`にnested objectや非文字列値がある場合は黙って破棄せず生成を失敗させる。絶対/UNC path、raw source file名を含む値も受理しない。Public-safe candidateは公開ID中心のfield形状に加え、Public Evidence Indexと同じ禁止文字列scanを通し、raw command、変数token、script tag等がnotesや表示fieldへ残った入力を拒否する。Extractionは既存semantic validatorを通し、`evidenceIndex`をNormalized blockのstory/episode/sceneと照合した上で、candidateの`id`、`type`、`confidence`と`evidenceIds`の対応だけを使う。candidate名、属性、`fields`、LLM生出力を複製しない。
...
前後contextは既定で空とする。Phase 5.3 generatorは`context.before`と`context.after`を常に空配列で生成する。前後contextの自動収集やCLI opt-inは未実装とする。将来追加する場合も、次をhard limitとする。
...
Phase 5.3 generatorは書き込み前に、Phase 5.2 validatorは既存bundleまたはselection fileを読み込む前に、該当する次の境界をすべて確認する。
...
Phase 5.2 validatorはbundle内の次を検証する。Phase 5.3 generatorはsafe `reports/validation.json`を自ら作成し、temporary directoryへbundleを書き出した後、atomic rename前に同じ検証を通す。
...
validatorはread-onlyのため、validation失敗時にもbundleを変更しない。Phase 5.3 generatorではvalidation失敗時にfinal bundleを作らないで、一時directoryを削除する。既存`packetId`を上書きせず、同一filesystem上のno-clobber atomic publishだけを許可する。いずれもstdout/stderrにはsafe aggregateだけを出す。一時directory削除にも失敗した場合は、固定root内のopaqueな`.tmp-<packetId>`相対pathだけを示してnon-zero終了する。
...
Phase 5.4の`manage_internal_review_evidence_packets.py inventory`は、fixed rootをfail-closedに検査した上でsafe aggregateだけを表示する。表示してよいのはopaque `packetId`、created/expires時刻、期限warning、story/entry/component件数、合計size、manifest digest、validationのaggregateだけである。raw text、raw command、内部ID、title、sourceKey、local pathは表示しない。

rootに未知entryまたはtemporary entryが存在する場合、entry名を表示せずconfig errorとして終了する。inventoryは期限切れをwarningとして表示するが、schema/validator failureへ昇格しない。network drive・同期folder・共有ACLの判定や強制は引き続きoperator責務である。

## 12.4 Cleanup

Phase 5.4のcleanupは`manage_internal_review_evidence_packets.py cleanup --packet-id <opaque-id> [--execute]`で行う。1回に指定できる`packetId`は1件だけであり、root全体、directory path、glob、期限切れPacket全件の削除は許可しない。次の安全策を持つ。

- dry-run既定、削除には`--execute`必須
- `packetId`を1件だけ明示し、root全体やglobだけの削除を許可しない
- generatorと同じresolve/symlink/junction/reparse point/Git checkを削除直前にも再実行し、検査できない・途中で状態が変わった場合は削除しない
- invalid Packet、unknown/temp entry、root外へresolveする対象はfail-closedでCLI削除しない
- 対象component数・合計size・manifest digestだけを事前表示し、raw/内部IDを表示しない
- 通常のfilesystem削除を行い、SSD・backup・journalに対するsecure eraseは保証しない

cleanup失敗時はroot外への移動や安易な手動削除へ切り替えない。safe error codeを確認し、生成・中断操作とfilesystem境界を調査する。削除後もhuman review noteとPublic Evidence Indexは残るが、Packet mapping/raw内容は残らない。後日の再照合は、保存済みの公開ID、review noteの`packetId`/manifest digest、再生成可能なsource snapshotを使う。
...
| inventoryでunknown/temp entryを検出 | 名称を表示せずexit 2 |
| cleanup対象がinvalidまたはroot外へresolve | 削除せずexit 1またはexit 2 |
...
| Phase 5.3: `internal-review-evidence-packet-generator` | Normalized Story + Public-safe candidate + 既存mappingからのbundle生成、cross-reference、safe report、atomic write、出力境界 | **完了** |
| Phase 5.4: `internal-review-evidence-packet-operations` | `Internal_Review_Evidence_Packet_Operations.md`、safe aggregate inventory、期限warning、1 packetId・dry-run既定cleanup CLI | **完了** |

Phase 5.2〜5.4は分離PRとした。Phase 5.2〜5.4は合成fixtureだけを使って完了し、Phase 5.3は実データPacketを生成していない。Phase 5.4のoperationsも`workspace/`限定・非commitである。review note取り込みや自動promotionはこれらのphaseへ便乗させない。
...
- `docs/runbooks/Internal_Review_Evidence_Packet_Operations.md`
