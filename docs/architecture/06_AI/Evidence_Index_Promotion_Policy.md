...
- ~~`episodeOrder`の正式な根拠とepisode追加・順序変更時の通常運用~~ → **`public-id-manifest-assignment-policy`で解決**: 人間確認済みmanifestの`episodeNumber`を正式な順序根拠とし、scriptのentry初出順は候補用ヒューリスティックに限定する。公開済みの途中挿入・順序変更はblockingとし、専用migration設計なしに既存IDを変更しない（`Public_ID_Registry_Design.md` §8、`docs/runbooks/Public_ID_Manifest_Assignment.md`）
