...
| Phase 16: `evidence-index-promotion-batch-policy`（本PR） | 複数storyへ広げる前のbatch promotion運用方針（batch size・Registry review条件・promotion前後チェックリスト・visual review・failed story/rollback・PR分割方針）を設計する | **完了（本PR、`docs/runbooks/Evidence_Index_Batch_Promotion_Policy.md`新設、実装変更なし）** |
