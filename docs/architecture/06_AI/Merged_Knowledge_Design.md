...
- Stage B自体は順序graphの整合性判定を行わない。merge前の複数Stage A documentを入力する独立CLI `scripts/check_timeline_consistency.py`が、`relative_order`の`same_time`同値class縮約・class内before/after矛盾・class間有向循環、episode metadata由来`explicit_order`の同一episode/field値競合、同一story内の一意な`canonicalOrder`とrelative constraintの不整合をcandidate/Evidence/`extractionRun` provenance付きwarningとして検出し、story単位のcanonical review準備状況をinformationalに監査する。release/displayによる補完、cross-story比較、総順序判定と順序確定は引き続き対象外とする（`docs/runbooks/Timeline_Consistency_Check.md`）

---

# 7.5 Special Speaker Label merge方針（Speaker Label Normalization設計）

`name`コマンド/`@ChTalkName`由来のspeaker labelのうち`labelType`が`single_speaker`以外のもの（`Extraction_Result_Schema.md` §13.5 `SpecialSpeakerLabelCandidate`）は、**通常のCharacter merged entityとは別枠（`entities.specialSpeakerLabels`）として組み立てる**。Character entityへは一切混ぜない（`agents/merger/speaker_labels.py`）。

| 項目 | 方針 |
|---|---|
| merge key | candidate単位（Character候補の「名前のみ」ケース、§5.1第3分岐と同じ`("unresolved", candidate["id"])`）。同一rawLabelでもエピソードをまたいだ自動統合はしない（名前一致だけで確定しない原則、§4.1原則2） |
| status/canonicalId | **常に`status: unresolved`・`canonicalId: null`固定**。`inferredSpeakers`にconfirmed character dictionaryとの一致候補があっても、自動でconfirmed/mergedへ昇格しない |
| confidence集約 | 他entityと同じくmax（§4.3） |
| evidence集約 | 通常entityと同じ（`evidenceRefs`/`sourceCandidates`をそのまま保持） |
| report | `report.specialSpeakerLabelSummary`（labelType別/resolutionStatus別件数）を追加する。既存の`mergedEntityCounts`/`unresolvedEntityCounts`/`candidateCounts`（8種固定・schema上additionalProperties: false）には一切含めない、独立した集計として扱う |
| manual correction | 将来、人間レビューでcomponentsを実在キャラクターへ確定させる場合もmanual override（§8）経由で行う。このモジュール自体はconfirmed化の手段を提供しない |
