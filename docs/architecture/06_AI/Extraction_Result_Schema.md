...
複数documentのTimeline candidateはStage B merge後ではなく、Stage A candidate単位のまま`scripts/check_timeline_consistency.py`で横断検査する。`relative_order`は`same_time`を無向の同値関係として推移縮約し、同一class内の`before` / `after`とclass間の有向循環をwarning findingとして検出する。episode metadata由来の`explicit_order`は同一episode・同一`orderField`に複数値がある場合だけwarning findingとし、異なるfieldを比較せずwinnerも選ばない。関与する全candidate、Evidence、`extractionRun`を独立reportへ保持する。参照先episodeが入力に無い部分batchや検査不能候補は矛盾と推測せず理由付きで残す（`docs/runbooks/Timeline_Consistency_Check.md`）。

---

# 13.5 SpecialSpeakerLabelCandidate（Speaker Label Normalization設計）

`name`コマンド/`@ChTalkName`由来のspeaker labelのうち、`agents/parser/speaker_labels.py`の構造化結果（`Normalized_Story_JSON.md` §14.1参照）が`single_speaker`以外と判定したもの（speaker group・modifier付き・generic/ambiguousな表記）を対象とする。**通常のCharacterCandidateとは別配列（`specialSpeakerLabelCandidates`）に出力し、混ぜない。**

```json
{
  "id": "EVT_TEST_E01_CAND_SSL001",
  "type": "special_speaker_label_candidate",
  "sourceType": "script",
  "confidence": 0.5,
  "evidenceIds": ["EVT_TEST_E01_DLG0001"],
  "extractionRun": {
    "...": "§3.2と同一構造"
  },
  "rawLabel": "セイナ＆イヴ",
  "labelSource": "name_command",
  "labelType": "speaker_group",
  "components": ["セイナ", "イヴ"],
  "modifier": null,
  "baseLabel": null,
  "inferredSpeakers": [],
...
| Field | 必須 | 型 | 説明 |
|---|---:|---|---|
| `rawLabel` | Yes | string | DEC由来のspeaker labelそのまま（未加工） |
| `labelSource` | No | string \| null | `name_command`/`ch_talk_name`等、由来コマンド |
| `labelType` | Yes | string | `single_speaker`/`speaker_group`/`speaker_with_modifier`/`speaker_group_with_modifier`/`generic_speaker`/`ambiguous_speaker`/`unknown` |
| `components` | No | string[] | speaker group/modifier付きの構成要素名 |
| `modifier` | No | string \| null | 発話演出modifier（「小声」等） |
| `baseLabel` | No | string \| null | modifier除去後のbase名（`speaker_with_modifier`/`speaker_group_with_modifier`のみ設定） |
| `inferredSpeakers` | No | object[] | confirmed character dictionaryとの参考照合結果（`matchedName`/`characterId`/`matchStatus`/`confidence`）。**自動でresolutionStatusをconfirmedにする根拠にはしない** |
| `resolutionStatus` | Yes | string | `not_applicable`/`needs_review`/`inferred`/`confirmed`。自動付与は`not_applicable`/`needs_review`/`inferred`のみで、`confirmed`は将来の人間レビュー結果取り込み用に予約するのみ |

同一episode内で同一`rawLabel`を持つ発話は1件のcandidateへ統合し、発言したBlock IDをすべて`evidenceIds`へ集める（CharacterCandidateと同じ集約方針）。`labelType: single_speaker`（通常の単独キャラクター名）はこの配列の対象外とし、従来通りCharacterCandidate側で扱う。
