...
- ~~ローカルOllamaが起動していること（生成stepでのみ必要。`docker-compose.yml`の`ollama`サービス、または devcontainer外で起動したローカルOllama）~~ **（2026-07-20更新: 標準providerはOpenAI Codex CLIへ変更。ローカルLLMは品質評価により標準・予備いずれからも除外された。本行はStage 0/1 PoC実施当時の前提として保持する。標準経路は§17参照）**
...
| 3 | 生成（Episode Summary→Story Summary合成） | `scripts/generate_story_summaries.py`（ローカルLLM経路。標準は§17のCodex CLI手動経路） | ~~ローカルOllama必要~~ 標準はCodex CLI（人間が手動実行、§17参照） |
...
**（2026-07-20更新）** 本Stepが説明する`generate_story_summaries.py`によるローカルOllama自動生成は、標準経路ではなくなった。標準はOpenAI Codex CLIによる手動生成であり、詳細は§17を参照。以下はローカルLLM経路（代替・検証用）の手順として保持する。
...
- [ ] **(e) 未解決話者ラベルの人物名としての混入**: parserが未解決話者に付与するplaceholder（prompt側では固定表記「話者不明」に正規化済み）を、要約文がまるで実在の人物名であるかのように書いていないか（Stage 1 small batchで実際に検出された誤り。prompt側の対策後もLLMが「話者不明」を固有名詞として扱う可能性は残るため、レビューで確認する）
...
**追記（2026-07-20）**: あらすじ粒度に切り替えた後の評価でも、ローカルLLM（qwen2.5:14b/qwen3.6:35b-a3b）は班長=主人公の帰属誤り・幻覚evidenceRef・多言語混入が解消されず、標準・予備いずれからも除外された（§17参照）。本節が記録する傾向は、その判断の実測根拠の一部である。
...
# 17. 標準経路（2026-07-20確定: Provider=Codex・粒度=あらすじ）

## 17.1 背景・経緯

2026-07-18〜20のユーザー評価で、ローカルLLM（qwen2.5:14b/qwen3.6:35b-a3b）はあらすじ粒度で生成した場合でも、班長=主人公の帰属誤り・幻覚evidenceRef・多言語混入が残り実用未満と判断された。Claude Code内生成は高品質だが、開発方針自体のCodex移行に合わせ、標準providerは**OpenAI Codex CLI（ChatGPTサブスクリプション同梱）**に確定した。Claude Codeによる生成は代替として引き続き利用可能。ローカルLLM（Ollama、§2前提・§3 Pipeline overviewのStep 3行・§6 Step 3導入部が説明する`generate_story_summaries.py`経由の自動生成経路）は標準・予備いずれからも除外された。**§2前提・§3 Pipeline overview・§6 Step 3が説明するローカルOllama前提は、この決定により標準ではなくなった。**当該記述はStage 0/1 PoC実施当時の手順記録として保持するが、新規batchでは本節の標準経路に従う。

## 17.2 標準粒度: あらすじ水準（synopsis）

- Story Summary: 3文以内・約150字以内
- Episode Summary: 2文以内・約80字以内
- 目的は「読者がその話を読むか判断するための短いあらすじ」であり、本文の圧縮リライト（retelling）ではない。骨子のみを残し、細かい経緯・脇役固有名詞は省く
- 実測圧縮率の目安は本文セリフ全文の約10〜14%（詳細版は43〜54%相当だったが、これは本末転倒と判断され不採用となった）

## 17.3 ドメイン前提の必須注入

生成手段によらず、以下2点を必ず注入する。

1. 主人公はプレイヤーで作中「班長」と呼ばれる（役職名ではなく主人公本人であり、名前付きキャラクターと混同しない）
2. 話者不明モノローグは班長の内心の可能性が高く、行動主体を安易に近くの名前付きキャラクターへ帰属させない

上記は`knowledge/dictionaries/summary_domain_context.yaml`（既存・commit済み、§6.1参照）に記載済みである。`generate_story_summaries.py`（ローカルLLM経路）は既定で自動注入するが、**Codex CLIによる手動生成では自動注入が働かないため、Codex用プロンプトへ同内容を明記すること**。

## 17.4 標準生成フロー（7ステップ）

人間レビューを最終gateとする以下のフローを標準とする。既存の8ステップ構成（§3〜§11）とほぼ同一だが、Step 3（生成）が「scriptによるローカルOllama自動生成」から「人間によるCodex CLI手動実行」へ変わる点が主な違いである。

| # | 内容 | 対応する既存Step |
|---|---|---|
| 1 | 対象storyのnormalized/extraction/mergedを用意する | §5 Step 2相当（extraction/merged collectionも用意する点を拡張） |
| 2 | Codex用プロンプト（入力・§17.3のドメイン前提・§17.2のあらすじ粒度ルール・出力スキーマ・publicStoryId対応表を含む）を用意し、ユーザーがCodex CLIで実行する | §6 Step 3に相当するが手動実行 |
| 3 | `scripts/check_story_summary_drafts.py --normalized ...`でquality gate PASSを確認する | §7 Step 4と同一 |
| 4 | 人間が本文レビューする（レビュー観点は§17.5） | §8 Step 5と同一（レビュー観点を§17.5で拡張） |
| 5 | `review.status: approved`・`review.reviewer`を記録する | §8.2の修正手順と同一 |
| 6 | Public-safe projection → `promote_story_summaries.py`で`knowledge/summaries/stories/`へcommitする | §9 Step 6・§10 Step 7と同一 |
| 7 | 昇格後検証を行う | §11 Step 8と同一 |

Codex用プロンプトの実例雛形は`workspace/summary_drafts/CODEX_PROMPT.md`（workspace限定、非commit）にある。内容そのものはここへ転記しないが、少なくとも入力（normalized/extraction/merged）・§17.3のドメイン前提・§17.2の粒度ルール・出力スキーマ・publicStoryId対応表の5点を含めること。

## 17.5 人間レビューチェックリスト（標準版）

実データ評価で判明した観点を踏まえ、§8.1のPoC版チェックリスト（(a)〜(e)）を補う形で以下を確認する。あらすじ粒度での生成ではPoC時と異なる誤りパターン（班長=主人公の帰属誤り・幻覚evidenceRef・多言語混入）が確認されているため、レビュー時は特に(a)〜(d)を重点確認する。

- [ ] **(a) 班長=主人公の帰属**: 主人公（プレイヤー、作中「班長」）への帰属が正しく、名前付きキャラクターと混同していないか
- [ ] **(b) 話者不明モノローグの帰属**: 話者不明モノローグを誤って近くの名前付きキャラクターへ帰属していないか
- [ ] **(c) 創作・幻覚の混入**: 本文に無い人物・技名・設定を創作（造語・幻覚）していないか
- [ ] **(d) 多言語混入・文字化け**: 出力に日本語以外の言語混入や文字化けが無いか
- [ ] **(e) block ID/括弧書き参照の残留**: 本文中にblock ID・括弧書き参照が残っていないか（quality gateの盲点として、生セリフの逐語引用漏れも合わせて確認する）
- [ ] **(f) あらすじ粒度**: §17.2の目安（Story約150字・Episode約80字）を大きく超えていないか
- [ ] **(g) evidenceRefsの妥当性**: `evidenceRefs`が要約の骨子となる主張を支えているか

修正手順・quality gate再実行は§8.2をそのまま適用する。

## 17.6 関連

- `knowledge/dictionaries/summary_domain_context.yaml`（§17.3が注入を必須とするドメイン前提の格納場所）
- `workspace/summary_drafts/CODEX_PROMPT.md`（Codex用プロンプトの実例雛形、workspace限定・非commit）
- `scripts/check_story_summary_drafts.py`（§17.4 Step 3のquality gate、既存§7と同一script）
- `scripts/promote_story_summaries.py`（§17.4 Step 6の昇格、既存§10と同一script）
- `docs/architecture/06_AI/Story_Summary_Generation_Plan.md` § 13（本節の決定を1〜2段落で要約した記録）
